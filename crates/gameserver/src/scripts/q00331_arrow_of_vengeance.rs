...
fn chance(npc_id: i32) -> i32 {
    match npc_id { 20145 => 59, 20158 => 61, 20176 => 60, _ => 0 }
}
pub struct Q00331ArrowOfVengeance;
impl QuestScript for Q00331ArrowOfVengeance {
    fn id(&self) -> i32 { 331 }
    fn name(&self) -> &'static str { "Q00331_ArrowOfVengeance" }
    fn html_dir(&self) -> &'static str { "quests/Q00331_ArrowOfVengeance" }
    fn start_npcs(&self) -> &[i32] { &[BELTON] }
    fn talk_npcs(&self) -> &[i32] { &[BELTON] }
    fn kill_npcs(&self) -> &[i32] { &KILL_NPCS }
    fn quest_items(&self) -> &[i32] { &[HARPY_FEATHER, MEDUSA_VENOM, WYRMS_TOOTH] }
    ...
    fn on_event(&self, ctx: &mut QuestCtx, event: &str) -> Option<String> {
        ...
        match event {
            "30125-03.htm" => { ctx.start_quest(); Some(event.to_string()) }
            "30125-06.html" => { ctx.exit_quest(true, true); Some(event.to_string()) }
    ...
    fn on_kill(&self, ctx: &mut QuestCtx) {
        ...
        if ctx.roll(100) < chance(ctx.npc_id) {
            let item = match ctx.npc_id { 20145 => HARPY_FEATHER, 20158 => MEDUSA_VENOM, 20176 => WYRMS_TOOTH, _ => return };
    ...
    fn on_talk(&self, ctx: &mut QuestCtx) -> Option<String> {
        ctx.ensure_qs();
        if ctx.is_created() {
            return Some(if ctx.player_level() < MIN_LEVEL { "30125-01.htm" } else { "30125-02.htm" }.to_string());
        }
        if ctx.is_started() {
            ...
            if feathers + venoms + teeth > 0 {
                let bonus = if feathers + venoms + teeth >= 10 { 1000 } else { 0 };
