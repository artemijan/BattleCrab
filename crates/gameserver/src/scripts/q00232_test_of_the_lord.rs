...
fn varkees_talk(ctx: &mut QuestCtx) -> String {
        ...
        && !has(ctx, HUGE_ORC_FANG)
    {
        ...
        maybe_cond2(ctx, AXE_OF_CEREMONY, SWORD_INTO_SKULL, HANDIWORK_SPIDER_BROOCH, MONSTER_EYE_WOODCARVING);
...
fn tantus_talk(ctx: &mut QuestCtx) -> String {
    if has(ctx, ORDEAL_NECKLACE) && !has(ctx, AXE_OF_CEREMONY) && !has(ctx, TANTUS_CHARM) {
        ...
        if !has(ctx, NERUGA_AXE_BLADE) || ctx.quest_items_count(BONE_ARROW) < 1000 {
            ...
            maybe_cond2(ctx, HUGE_ORC_FANG, SWORD_INTO_SKULL, HANDIWORK_SPIDER_BROOCH, MONSTER_EYE_WOODCARVING);
...
fn hatos_talk(ctx: &mut QuestCtx) -> String {
    if has(ctx, ORDEAL_NECKLACE) && !has(ctx, SWORD_INTO_SKULL) && !has(ctx, HATOS_CHARM) {
        ...
        if has(ctx, URUTU_BLADE) && ctx.quest_items_count(TIMAK_ORC_SKULL) >= 10 {
            ...
            maybe_cond2(ctx, HUGE_ORC_FANG, AXE_OF_CEREMONY, HANDIWORK_SPIDER_BROOCH, MONSTER_EYE_WOODCARVING);
...
fn takuna_talk(ctx: &mut QuestCtx) -> String {
    ...
    } else if has(ctx, ORDEAL_NECKLACE) && has(ctx, TAKUNA_CHARM) && !has(ctx, HANDIWORK_SPIDER_BROOCH)
    {
        if ctx.quest_items_count(MARSH_SPIDER_FEELER) >= 10
            && ctx.quest_items_count(MARSH_SPIDER_FEET) >= 10
        {
            ...
            maybe_cond2(ctx, HUGE_ORC_FANG, AXE_OF_CEREMONY, SWORD_INTO_SKULL, MONSTER_EYE_WOODCARVING);
            "30641-04.html".to_string()
        } else {
            "30641-03.html".to_string()
        }
    } else if has(ctx, ORDEAL_NECKLACE) && has(ctx, HANDIWORK_SPIDER_BROOCH) && !has(ctx, TAKUNA_CHARM)
    {
...
fn chianta_talk(ctx: &mut QuestCtx) -> String {
    ...
    } else if has(ctx, ORDEAL_NECKLACE) && has(ctx, CHIANTA_CHARM) && !has(ctx, MONSTER_EYE_WOODCARVING)
    {
        if ctx.quest_items_count(ENCHANTED_MONSTER_CORNEA) < 20 {
            ...
            maybe_cond2(ctx, HUGE_ORC_FANG, AXE_OF_CEREMONY, SWORD_INTO_SKULL, HANDIWORK_SPIDER_BROOCH);
            "30642-04.html".to_string()
        }
    } else if has(ctx, ORDEAL_NECKLACE) && has(ctx, MONSTER_EYE_WOODCARVING) && !has(ctx, CHIANTA_CHARM)
    {
