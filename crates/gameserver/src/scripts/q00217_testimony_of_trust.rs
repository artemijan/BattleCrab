...
fn reagent_kill(ctx: &mut QuestCtx, cap: i64, intermediate: i32, final_item: i32, others: [i32; 2]) {
    if ctx.memo_state() == 6
...
impl QuestScript for Q00217TestimonyOfTrust {
    ...
    fn on_kill(&self, ctx: &mut QuestCtx) {
        ...
        match ctx.npc_id {
            DRYAD | DRYAD_ELDER => {
                if memo == 2 {
                    let flag = ctx.get_var("flag").and_then(|s| s.parse().ok()).unwrap_or(1);
            ...
            ANT_RECRUIT | ANT_GUARD => {
                reagent_kill(ctx, 5, GIANT_APHID, HONEY_DEW, [BASILISK_PLASMA, STAKATO_ICHOR]);
            }
            ANT_PATROL | ANT_SOLDIER | ANT_WARRIOR_CAPTAIN => {
                reagent_kill(ctx, 10, GIANT_APHID, HONEY_DEW, [BASILISK_PLASMA, STAKATO_ICHOR]);
            }
            MARSH_STAKATO | MARSH_STAKATO_WORKER => {
                reagent_kill(ctx, 10, STAKATOS_FLUIDS, STAKATO_ICHOR, [BASILISK_PLASMA, HONEY_DEW]);
            }
            MARSH_STAKATO_SOLDIER | MARSH_STAKATO_DRONE => {
                reagent_kill(ctx, 5, STAKATOS_FLUIDS, STAKATO_ICHOR, [BASILISK_PLASMA, HONEY_DEW]);
            }
            GUARDIAN_BASILISK => {
                reagent_kill(ctx, 10, BLOOD_OF_GUARDIAN_BASILISK, BASILISK_PLASMA, [STAKATO_ICHOR, HONEY_DEW]);
            }
            WINDSUS => {
