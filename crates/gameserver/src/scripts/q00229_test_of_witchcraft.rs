...
impl QuestScript for Q00229TestOfWitchcraft {
    ...
    fn attack_npcs(&self) -> &[i32] {
        &[NAMELESS_REVENANT, SKELETAL_MERCENARY, DREVANUL_PRINCE_ZERUEL]
    }
    ...
    fn on_kill(&self, ctx: &mut QuestCtx) {
        ...
        match ctx.npc_id {
            ...
            | LETO_LIZARDMAN_WARRIOR | LETO_LIZARDMAN_SHAMAN | LETO_LIZARDMAN_OVERLORD => {
                ingredient(ctx, LETO_LIZARDMAN_CHARM)
            }
            ...
            NAMELESS_REVENANT => {
                if has(ctx, ALEXANDRIAS_BOOK) && has(ctx, LARAS_MEMO) && !has(ctx, AKLANTOTH_3RD_GEM)
                {
...
fn evert_talk(ctx: &mut QuestCtx) -> String {
    ...
    } else if has(ctx, SOULTRAP_CRYSTAL) && has(ctx, BRIMSTONE_2ND) && !has(ctx, ZERUEL_BIND_CRYSTAL)
    {
        // TODO(G22): getSummonedNpcCount()<1 guard; spawn Zeruel.
        ctx.spawn_attacker(DREVANUL_PRINCE_ZERUEL, true);
        "30633-02.htm".to_string()
    } else if has(ctx, ZERUEL_BIND_CRYSTAL) && !has(ctx, SOULTRAP_CRYSTAL) && !has(ctx, BRIMSTONE_2ND)
    {
