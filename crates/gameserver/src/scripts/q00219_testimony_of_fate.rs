...
impl QuestScript for Q00219TestimonyOfFate {
    ...
    fn on_event(&self, ctx: &mut QuestCtx, event: &str) -> Option<String> {
        ...
        match event {
            ...
            "30419-05.html" => {
                if has(ctx, ARKENIAS_NOTE) && has(ctx, RED_FAIRY_DUST) && has(ctx, BLIGHT_TREANT_SAP)
                {
    ...
    fn on_kill(&self, ctx: &mut QuestCtx) {
        ...
        match ctx.npc_id {
            ...
            MARSH_STAKATO | MARSH_STAKATO_WORKER | MARSH_STAKATO_SOLDIER | MARSH_STAKATO_DRONE => {
                herb_kill(ctx, NIGHTSHADE_ROOT, [MEDUSAS_ICHOR, MARSH_SPIDER_FLUIDS, DEAD_SEEKER_DUNG, TYRANTS_BLOOD]);
            }
            MEDUSA => herb_kill(
                ctx,
                MEDUSAS_ICHOR,
                [MARSH_SPIDER_FLUIDS, DEAD_SEEKER_DUNG, TYRANTS_BLOOD, NIGHTSHADE_ROOT],
            ),
            TYRANT | TYRANT_KINGPIN => herb_kill(
                ctx,
                TYRANTS_BLOOD,
                [MEDUSAS_ICHOR, MARSH_SPIDER_FLUIDS, DEAD_SEEKER_DUNG, NIGHTSHADE_ROOT],
            ),
            DEAD_SEEKER => herb_kill(
                ctx,
                DEAD_SEEKER_DUNG,
                [MEDUSAS_ICHOR, MARSH_SPIDER_FLUIDS, TYRANTS_BLOOD, NIGHTSHADE_ROOT],
            ),
            MARSH_SPIDER => herb_kill(
                ctx,
                MARSH_SPIDER_FLUIDS,
                [MEDUSAS_ICHOR, DEAD_SEEKER_DUNG, TYRANTS_BLOOD, NIGHTSHADE_ROOT],
            ),
...
fn treant_talk(ctx: &mut QuestCtx) -> String {
    ...
    } else if has(ctx, TIMIRIRAN_SEED) && !has(ctx, BLIGHT_TREANT_SAP) && !has(ctx, BLACK_WILLOW_LEAF)
    {
        "31850-03.html".to_string()
    } else if has(ctx, TIMIRIRAN_SEED) && has(ctx, BLACK_WILLOW_LEAF) && !has(ctx, BLIGHT_TREANT_SAP)
    {
