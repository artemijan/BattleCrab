...
///
/// The **full** `AbnormalVisualEffect` enum, generated from the Java source.
/// It used to stop at id 38, which silently dropped 102 of the names the dist's
/// own skills reference (`AURA_BUFF`, `ABSORB_SHIELD`, the `*_CHANGE` grade
/// glows…) — the skill parsed, but its visual never reached the client. An
/// unknown name still yields `None` and is simply not shown, matching Java's
/// `findByName` + warning. `VP_KEEP` shares client id 29 with `VP_UP` in Java,
/// comment and all.
