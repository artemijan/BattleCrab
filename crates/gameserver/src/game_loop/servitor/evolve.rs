...
fn do_evolve(
    ...
    let Some(pet_oid) = crate::game_loop::servitor::pet_of(world, player_oid) else {
        return false;
    };
...
fn summon_evolved(
    ...
    let Some(pet_oid) = crate::game_loop::servitor::summon_pet(world, player_oid) else {
        return false;
    };
    ...
    if let Some(pos) = at {
        ...
        crate::game_loop::space::visibility::update_npc_region(world, pet_oid);
    }
    // `item.setEnchantLevel(petSummon.getLevel())` — the collar records the
    // level, which is what a later `restore` reads back out.
    crate::game_loop::servitor::sync_collar_enchant(world, player_oid, pet_oid);
