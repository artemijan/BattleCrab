...
/// The dist catalogues, parsed once per test binary.
///
/// `load_from` walks `dist/game/data` and parses the XML from scratch every
/// call — 610 ms for `SkillData`, ~160 ms each for `ItemData` and `NpcData` —
/// and several hundred call sites across these tests were each paying that
/// again. Cloning an already-parsed catalogue is 16–35× cheaper (4.6 ms for
/// `ItemData`, 37 ms for `SkillData`), so a test that needs its own mutable
/// copy (`insert_for_test`, or handing it to `world.data`) takes a clone, and
/// one that only reads borrows the `'static` directly.
///
/// `LazyLock` parses on first use, so a filtered `cargo test` run that touches
/// none of these still pays nothing.
pub(super) mod dist {
    ...
    macro_rules! cached {
        ($borrow:ident, $ty:ty) => {
            ...
            pub fn $borrow() -> &'static $ty {
                static CELL: LazyLock<$ty> = LazyLock::new(|| <$ty>::load_from(DIST));
                &CELL
            }
        };
...
async fn character_create_inserts_into_real_schema() {
    ...
    let data = GameData {
        ...
        castle_siege_guards: crate::data::castle_siege_guards::CastleSiegeGuards::empty(),
