...
fn points_of(world: &World, oid: i32) -> i32 {
    world.objects.get_component::<crate::model::Player>(&oid).unwrap().raidboss_points
}

fn seed_damage(world: &mut World, npc: i32, dealer: i32, dmg: f64) {
    world.objects.get_component_mut::<crate::model::npc::AggroList>(&npc).unwrap().0.entry(dealer).or_default().damage =
        dmg;
}
...
fn a_party_splits_the_raid_points() {
    ...
    assert_eq!(points_of(&world, mate), 50, "including the member who dealt no damage");
}
...
fn a_distant_party_member_earns_no_raid_points() {
    ...
    seed_damage(&mut world, BOSS, PLAYER, 500.0);
    make_party(&mut world, &[PLAYER, mate], LootRule::FindersKeepers);
    world.objects.get_component_mut::<Position>(&mate).unwrap().x += 100_000;

    crate::game_loop::death::npc_do_die(&mut world, BOSS, PLAYER);
    assert_eq!(points_of(&world, mate), 0, "out of range, no points");
    assert_eq!(points_of(&world, PLAYER), 100, "and the one in range takes the whole share");
}
...
fn the_real_datapack_parses_raid_points() {
    ...
    let with_points = (1..=30000).filter(|id| npcs.get(*id).is_some_and(|t| t.raid_points > 0.0)).count();
    assert!(with_points > 100, "expected many raid-point NPCs, got {with_points}");
}
