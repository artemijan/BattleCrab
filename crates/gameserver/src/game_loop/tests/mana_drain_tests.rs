...
fn mana_dam_follows_the_java_formula() {
    ...
    let d = formulas::calc_mana_dam(100.0, 50.0, 970.0, 20.0, 1.0, MagicFailure::None, false, 7000.0);
...
fn a_bigger_mp_pool_is_drained_harder() {
    let small = formulas::calc_mana_dam(100.0, 50.0, 500.0, 20.0, 1.0, MagicFailure::None, false, 7000.0);
    let big = formulas::calc_mana_dam(100.0, 50.0, 1000.0, 20.0, 1.0, MagicFailure::None, false, 7000.0);
    assert!((big - small * 2.0).abs() < 1e-9, "double the pool, double the drain: {small} -> {big}");
}
...
fn a_crit_triples_the_drain_and_then_clamps_to_the_limit() {
    let base = formulas::calc_mana_dam(100.0, 50.0, 970.0, 20.0, 1.0, MagicFailure::None, false, 7000.0);
    let crit = formulas::calc_mana_dam(100.0, 50.0, 970.0, 20.0, 1.0, MagicFailure::None, true, 7000.0);
    assert!((crit - base * 3.0).abs() < 1e-9, "×3 when under the cap");

    // Same crit against a 100 cap: clamped, not tripled.
    let capped = formulas::calc_mana_dam(100.0, 50.0, 970.0, 20.0, 1.0, MagicFailure::None, true, 100.0);
    assert!((capped - 100.0).abs() < 1e-9, "clamped to criticalLimit, got {capped}");
}
...
fn spiritshots_scale_matk_before_the_square_root() {
    let plain = formulas::calc_mana_dam(100.0, 50.0, 970.0, 20.0, 1.0, MagicFailure::None, false, 7000.0);
    let sps = formulas::calc_mana_dam(100.0, 50.0, 970.0, 20.0, 2.0, MagicFailure::None, false, 7000.0);
    assert!((sps - plain * 2.0_f64.sqrt()).abs() < 1e-9, "×sqrt(2), not ×2: {plain} -> {sps}");
}
...
fn a_resisted_drain_is_halved_not_floored() {
    let full = formulas::calc_mana_dam(100.0, 50.0, 970.0, 20.0, 1.0, MagicFailure::None, false, 7000.0);
...
fn magic_affected_compares_attack_against_defence() {
    ...
    assert!(!formulas::calc_magic_affected(100.0, 200.0, 0.0), "a dead heat does not land");
    // A clear mAtk edge lands.
    assert!(formulas::calc_magic_affected(100.0, 50.0, 0.0), "more attack than defence lands");
    // And a clear deficit does not.
    assert!(!formulas::calc_magic_affected(10.0, 500.0, 0.0), "far more defence than attack fails");
}
...
fn the_gaussian_can_flip_the_verdict_either_way() {
    ...
    assert!(formulas::calc_magic_affected(100.0, 0.0, 0.0), "baseline lands");
    assert!(!formulas::calc_magic_affected(100.0, 0.0, -4.0), "a bad draw loses it");
    // Overwhelming defence, but a good enough draw lands it.
    assert!(!formulas::calc_magic_affected(10.0, 500.0, 0.0), "baseline fails");
    assert!(formulas::calc_magic_affected(10.0, 500.0, 4.0), "a good draw wins it");
}
...
fn a_zero_attack_actor_never_lands_the_drain() {
    assert!(!formulas::calc_magic_affected(0.0, 0.0, 5.0), "no attack, no drain, whatever the roll");
}
...
fn real_dist_mana_drain_skills_parse() {
    ...
    for (id, expected_limit) in [(1102, 1600.0), (1210, 1600.0), (1398, 7000.0), (1399, 7000.0)] {
        let skill = skills.get(id, 1).unwrap_or_else(|| panic!("skill {id} loads"));
        let found = skill.effects.iter().find_map(|e| match e {
            SkillEffect::MagicalAttackMp { power, critical, critical_limit } => Some((*power, *critical, *critical_limit)),
            _ => None,
        });
        let (power, critical, limit) = found.unwrap_or_else(|| panic!("skill {id} carries MagicalAttackMp"));
        assert!(power > 0.0, "skill {id} has a real power, got {power}");
        assert!(critical, "skill {id} declares critical=true");
        assert!((limit - expected_limit).abs() < 1e-9, "skill {id} criticalLimit {expected_limit}, got {limit}");
    }
}
...
fn the_two_nukes_carry_only_the_drain_effect() {
    ...
    for id in [1398, 1399] {
        ...
        assert_eq!(skill.effects.len(), 1, "skill {id} has exactly one effect: {:?}", skill.effects);
        assert!(matches!(skill.effects[0], SkillEffect::MagicalAttackMp { .. }));
    }
}
...
fn the_drain_skills_are_magic_skills() {
    ...
    for id in [1102, 1210, 1398, 1399] {
        assert_eq!(skills.get(id, 1).unwrap().magic_type, 1, "skill {id} is a magic skill");
    }
}
...
fn the_two_debuffs_and_the_two_nukes_differ_in_badness() {
    let skills = crate::data::skill_data::SkillData::load_from(DIST);
    for id in [1102, 1210] {
        assert!(skills.get(id, 1).unwrap().is_debuff, "skill {id} is a debuff");
    }
}
