...
/// **Custom `HeroAuraOnMounts`: the aura follows a hero onto the wyvern.**
/// The client binds both hero glows to the human mesh, so mounting drops them
/// however correct the packet is (Java included). With the toggle on, whoever
/// carries the glow on foot gets a stand-in abnormal visual while the model is
/// swapped — in the onlookers' `CharInfo` *and* in the rider's own
/// `ExUserInfoAbnormalVisualEffect` — and loses it again on dismount.
#[test]
fn hero_aura_on_mounts_substitutes_a_visual_effect() {
    const AURA_BUFF: i16 = 57;
    let (mut world, ..) = admin_world();
    world.data.pet_data = crate::data::pet_data::PetData::load_from(DIST);
    world.data.npc_data = crate::data::npc_data::NpcData::load_from(DIST);
    world.data.gm.hero_aura = true;
    world.data.gm.hero_aura_on_mounts = Some(AURA_BUFF);
    let mut gm_rx = ingame_player_access(&mut world, 1, 8970, 100);
    let mut ob_rx = ingame_player_access(&mut world, 2, 8971, 0);
    drain(&mut gm_rx);
    drain(&mut ob_rx);

    let visuals = |world: &World| crate::game_loop::abnormal::visual_effects(world, 8970);
    assert!(
        !visuals(&world).contains(&AURA_BUFF),
        "on foot the real hero glow does the job — no stand-in"
    );

    on_packet(&mut world, 1, build_admin("ride_wyvern"));
    assert!(
        visuals(&world).contains(&AURA_BUFF),
        "mounted, the stand-in aura is added"
    );
    // It reaches the onlooker's CharInfo…
    let ci = drain(&mut ob_rx)
        .into_iter()
        .rev()
        .find(|p| p[0] == server_packets::opcodes::CHAR_INFO)
        .expect("CharInfo");
    assert!(
        ci.windows(2).any(|w| w == AURA_BUFF.to_le_bytes()),
    ...
    assert!(
        drain(&mut gm_rx).iter().any(|p| {
            p[0] == server_packets::opcodes::EX
                ...
                && p.windows(2).any(|w| w == AURA_BUFF.to_le_bytes())
        }),
        "the rider's own client is told about it too"
    );

    // Landing hands the job back to the real glow.
    on_packet(&mut world, 1, build_admin("unride"));
    assert!(
        !visuals(&world).contains(&AURA_BUFF),
        "dismounted, the stand-in is gone again"
    );

    // A transform swaps the model the same way, so it applies there too.
    world
        .objects
        .get_component_mut::<Player>(&8970)
        .unwrap()
        .transform_display_id = 106;
    assert!(visuals(&world).contains(&AURA_BUFF), "transforms too");
