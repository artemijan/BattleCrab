...
/// A minimal weapon template for the augment tests (the fields the equip
/// path reads; everything else at its zero).
fn simple_weapon_template(item_id: i32) -> crate::data::item_data::ItemTemplate {
    use crate::data::item_data::{CrystalType, ItemHandler, ItemKind, ItemTemplate, SLOT_R_HAND};
    ItemTemplate {
        trade_flags: Default::default(),
        time: -1,
...
/// **An augment's active skill rides the equip** (G15.5): granted into the
/// skill book while the item is worn, gone when it comes off, and re-derived
/// at login — with the stale sweep dropping an augment skill the book
/// persisted past its item.
#[test]
fn augment_active_skill_rides_the_equip() {
    use crate::data::option_data::OptionEntry;
    ...
    world
        .data
        .item_data
        .insert_for_test(simple_weapon_template(600));
    let mut active = passive_clan_test_skill(3158);
    active.operate_type = crate::model::skill::OperateType::Active;
    world.data.skill_data.insert_for_test(active);
    world.data.options.insert_for_test(OptionEntry {
        id: 4001,
        active_skills: vec![(3158, 1)],
        ..Default::default()
    });
    {
        let World { objects, data, .. } = &mut world;
        let inv = objects.get_component_mut::<Inventory>(&3001).unwrap();
        inv.add_item(&data.item_data, 9500, 600, 1);
        inv.set_augmentation(9500, 8723, 4001, 0);
    }

    let knows = |world: &World| {
        world
            .objects
            .get_component::<SkillBook>(&3001)
            .is_some_and(|b| b.0.contains_key(&3158))
    };
    assert!(!knows(&world));
    items::handle_use_item(&mut world, 1, &use_item_body(9500));
    assert!(knows(&world), "equipping granted the active skill");
    items::handle_use_item(&mut world, 1, &use_item_body(9500));
    assert!(!knows(&world), "unequipping removed it");

    // Login: worn item re-grants; a stale persisted id without its item is
    // swept (the cursed-weapon pattern).
    items::handle_use_item(&mut world, 1, &use_item_body(9500)); // wear it again
    assert!(knows(&world));
    crate::game_loop::options::apply_worn_options_at_login(&mut world, 3001);
    assert!(
        knows(&world),
        "login re-derivation keeps a worn augment's skill"
    );
    // Strip the augment behind the book's back — the sweep must catch it.
    world
        .objects
        .get_component_mut::<Inventory>(&3001)
        .unwrap()
        .remove_augmentation(9500);
    crate::game_loop::options::apply_worn_options_at_login(&mut world, 3001);
    assert!(!knows(&world), "the stale augment skill was swept at login");
}

/// **An augment's activation skill fires on the configured action** (G15.5):
/// ATTACK-type on a plain hit only, at `Rnd.get(100) < chance`, applied to
/// the hit target with no cast time.
#[test]
fn augment_trigger_fires_on_a_plain_hit() {
    use crate::data::option_data::{OptionEntry, OptionSkillType, OptionTrigger};
    use crate::model::inventory::Inventory;

    let (mut world, ..) = test_world();
    let _rx = ingame_player(&mut world, 1, 3001, 0, 0, 0);
    let _rx2 = ingame_player(&mut world, 2, 3002, 50, 0, 0);
    world.id_pool = 0x4200_0000..0x4200_0100;
    world
        .data
        .item_data
        .insert_for_test(simple_weapon_template(600));
    // The triggered skill: a plain stat buff so the landing is observable.
    let mut buff = passive_clan_test_skill(3159);
    buff.operate_type = crate::model::skill::OperateType::Active;
    buff.abnormal_time = 30;
    ...
    });
    {
        ...
        let inv = objects.get_component_mut::<Inventory>(&3001).unwrap();
        inv.add_item(&data.item_data, 9500, 600, 1);
        inv.set_augmentation(9500, 8723, 4002, 0);
    }
    items::handle_use_item(&mut world, 1, &use_item_body(9500));

    let buffed = |world: &World| {
        world
            .objects
            .get_component::<crate::model::components::Buffs>(&3002)
            .is_some_and(|b| b.0.iter().any(|x| x.skill_id == 3159))
    };
    // A crit fires only CRITICAL-type — the ATTACK trigger stays quiet.
    crate::game_loop::options::fire_augment_attack_triggers(&mut world, 3001, 3002, true);
    assert!(!buffed(&world), "a crit does not fire an ATTACK trigger");
    // A plain hit rolls: 60 >= 50 misses, 10 < 50 fires.
    world.forced_rolls.push_back(60);
    crate::game_loop::options::fire_augment_attack_triggers(&mut world, 3001, 3002, false);
    assert!(!buffed(&world), "a failed roll fires nothing");
    world.forced_rolls.push_back(10);
    crate::game_loop::options::fire_augment_attack_triggers(&mut world, 3001, 3002, false);
    assert!(buffed(&world), "the trigger landed its skill on the target");

    // Unequip deregisters: no roll is even consumed.
    items::handle_use_item(&mut world, 1, &use_item_body(9500));
    world.forced_rolls.push_back(0);
    crate::game_loop::options::fire_augment_attack_triggers(&mut world, 3001, 3002, false);
    assert_eq!(
        ...
        1,
        "no trigger registered after unequip"
    );
}
