...
fn resist_multiplier_lowers_the_landing_rate() {
    ...
    let unresisted =
        calc_effect_land_rate(40, 50, 0, 40, 1.0, 1.0, 1.0, 0.0, 1.0, Default::default());
    assert!((unresisted - 80.0).abs() < 1e-9, "got {unresisted}");

    // Guts (x0.5): 80 * 0.5 = 40.
    let resisted =
        calc_effect_land_rate(40, 50, 0, 40, 0.5, 1.0, 1.0, 0.0, 1.0, Default::default());
    assert!((resisted - 40.0).abs() < 1e-9, "got {resisted}");

    // Touch of Death (x1.3): 80 * 1.3 = 104, clamped down to the 90 ceiling.
    let vulnerable =
        calc_effect_land_rate(40, 50, 0, 40, 1.3, 1.0, 1.0, 0.0, 1.0, Default::default());
    assert!(
    ...
    let crushed =
        calc_effect_land_rate(40, 50, 0, 40, 0.01, 1.0, 1.0, 0.0, 1.0, Default::default());
    ...
    assert_eq!(
        calc_effect_land_rate(40, -1, 0, 40, 0.01, 1.0, 1.0, 0.0, 1.0, Default::default()),
        100.0
    );
}
