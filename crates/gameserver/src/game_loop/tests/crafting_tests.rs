...
fn weapon_template(item_id: i32, name: &str) -> ItemTemplate {
    ItemTemplate { kind: ItemKind::Weapon, body_part: SLOT_R_HAND, ..etc_template(item_id, name, false, ItemHandler::None) }
}
...
fn install_fixtures(world: &mut World) {
    ...
    world.data.item_data.insert_for_test(etc_template(57, "Adena", true, ItemHandler::None));
    world.data.item_data.insert_for_test(etc_template(MATERIAL, "Iron Ore", true, ItemHandler::None));
    world.data.item_data.insert_for_test(etc_template(RECIPE_ITEM, "Recipe: Broadsword", false, ItemHandler::Recipes));
    world.data.item_data.insert_for_test(weapon_template(PRODUCT, "Broadsword"));
    world.data.item_data.insert_for_test(weapon_template(RARE_PRODUCT, "Broadsword*"));
    world.data.recipes.insert_for_test(recipe(RECIPE_LIST, 100, None));
    world.data.recipes.insert_for_test(recipe(RECIPE_LIST_FAIL, 0, None));
    world.data.recipes.insert_for_test(recipe(RECIPE_LIST_RARE, 100, Some(RareProduction { item_id: RARE_PRODUCT, count: 1, rarity: 100 })));
...
fn learn_skill(world: &mut World, oid: i32, skill_id: i32) {
    world.objects.get_component_mut::<SkillBook>(&oid).unwrap().0.insert(skill_id, 1);
}
...
fn using_recipe_item_registers_recipe() {
    ...
    assert!(world.objects.get_component::<RecipeBook>(&3001).unwrap().dwarven.contains(&RECIPE_LIST), "recipe registered");
    assert_eq!(world.objects.get_component::<Inventory>(&3001).unwrap().count_of(RECIPE_ITEM), 0, "recipe item consumed");
    let out = drain(&mut rx);
    assert!(out.iter().any(|p| p[0] == server_packets::opcodes::SYSTEM_MESSAGE && sm_id(p) == server_packets::sm_ids::S1_HAS_BEEN_ADDED), "S1_HAS_BEEN_ADDED sent");
}
...
fn learn_refused_without_craft_ability() {
    ...
    assert!(world.objects.get_component::<RecipeBook>(&3001).unwrap().dwarven.is_empty(), "not registered");
    assert_eq!(world.objects.get_component::<Inventory>(&3001).unwrap().count_of(RECIPE_ITEM), 1, "recipe item kept");
    let out = drain(&mut rx);
...
fn self_craft_success_consumes_and_rewards() {
    ...
    world.objects.get_component_mut::<RecipeBook>(&3001).unwrap().dwarven.push(RECIPE_LIST);
    ...
    let out = drain(&mut rx);
    assert!(
        out.iter().any(|p| p[0] == server_packets::opcodes::RECIPE_ITEM_MAKE_INFO && p[17] == 1),
        "RecipeItemMakeInfo success=1 sent"
    );
    assert!(has_sm(&out, server_packets::sm_ids::YOU_HAVE_EARNED_S1), "earned message");
}
...
fn self_craft_missing_material_aborts() {
    ...
    let out = drain(&mut rx);
    assert!(has_sm(&out, server_packets::sm_ids::YOU_NEED_S2_MORE_S1_S), "need-more message");
}
...
fn self_craft_failure_consumes_materials_no_product() {
    ...
    let mut rx = ingame_caster(&mut world, 1, 3001, 0, 0);
    learn_skill(&mut world, 3001, 172);
    world.objects.get_component_mut::<RecipeBook>(&3001).unwrap().dwarven.push(RECIPE_LIST_FAIL);
    give(&mut world, 3001, 9001, MATERIAL, 10);
    drain(&mut rx);
    ...
    let out = drain(&mut rx);
    assert!(has_sm(&out, server_packets::sm_ids::YOU_FAILED_AT_MIXING_THE_ITEM), "mixing-failed message");
}
...
fn self_craft_rolls_masterwork_rare() {
    ...
    let mut rx = ingame_caster(&mut world, 1, 3001, 0, 0);
    learn_skill(&mut world, 3001, 172);
    world.objects.get_component_mut::<RecipeBook>(&3001).unwrap().dwarven.push(RECIPE_LIST_RARE);
    give(&mut world, 3001, 9001, MATERIAL, 10);
...
fn manufacture_store_craft_for_customer() {
    ...
    crafting::handle_list_set(&mut world, 1, vec![cp::ManufactureLine { recipe_id: RECIPE_LIST, cost: 1000 }]);
    ...
    assert_eq!(world.objects.get_component::<Inventory>(&3001).unwrap().adena(), 1000, "manufacturer received fee");
    assert_eq!(pvit(&world, 3001).cur_mp, mp0 - 30.0, "manufacturer MP consumed");
