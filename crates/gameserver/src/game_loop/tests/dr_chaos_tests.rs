...
fn chaos_world() -> (World, db::CmdRx, tokio::sync::mpsc::UnboundedReceiver<LoginLinkCommand>) {
...
fn find(world: &World, npc_id: i32) -> Option<i32> {
    world
        ...
        .flatten()
        .copied()
        .find(|oid| world.objects.get_component::<crate::model::npc::Npc>(oid).is_some_and(|n| n.npc_id == npc_id))
}
...
fn boot_resolves_each_status() {
    ...
    assert!(world.objects.get_component::<DrChaosState>(&dc).is_some(), "paranoia armed");
    ...
    assert!(find(&world, DOCTOR_CHAOS).is_none(), "Dr. Chaos not up while the golem is");
    ...
    assert!(find(&world, DOCTOR_CHAOS).is_some(), "Dr. Chaos back after downtime");
}
...
fn a_lingering_player_triggers_the_transformation() {
    ...
    let dc_pos = world.objects.get_component::<Position>(&dc).copied().unwrap();
    let _rx = ingame_caster(&mut world, CID, PLAYER, dc_pos.x + 100, dc_pos.y);
    // Wind the timer down to 1 so one tick tips him over.
    world.objects.get_component_mut::<DrChaosState>(&dc).unwrap().pissed_off = 1;
    ...
    assert!(find(&world, DOCTOR_CHAOS).is_some(), "still on-screen mid-cinematic");
    assert!(find(&world, CHAOS_GOLEM).is_none(), "golem not up yet");
    advance_ticks(&mut world, 18 * 10);
    assert!(find(&world, DOCTOR_CHAOS).is_none(), "Dr. Chaos gone after beat 5");
    assert!(find(&world, CHAOS_GOLEM).is_some(), "the golem stands after the cinematic");
}
...
fn nobody_near_leaves_the_timer_alone() {
    ...
    dr_chaos::resolve_at_boot(&mut world);
    let dc = find(&world, DOCTOR_CHAOS).unwrap();
    world.objects.get_component_mut::<DrChaosState>(&dc).unwrap().pissed_off = 5;

    dr_chaos::handle_paranoia(&mut world, dc);

    assert_eq!(world.objects.get_component::<DrChaosState>(&dc).unwrap().pissed_off, 5, "no drain");
...
fn a_dead_player_does_not_drain() {
    ...
    world.objects.get_component_mut::<Vitals>(&PLAYER).unwrap().dead = true;
    world.objects.get_component_mut::<DrChaosState>(&dc).unwrap().pissed_off = 5;

    dr_chaos::handle_paranoia(&mut world, dc);
    assert_eq!(world.objects.get_component::<DrChaosState>(&dc).unwrap().pissed_off, 5, "the corpse doesn't count");
}
...
fn talking_drains_and_eventually_transforms() {
    ...
    dr_chaos::resolve_at_boot(&mut world);
    let dc = find(&world, DOCTOR_CHAOS).unwrap();
    let before = world.objects.get_component::<DrChaosState>(&dc).unwrap().pissed_off;

    let html = dr_chaos::on_first_talk(&mut world, dc);
    assert!(html.is_some(), "a paranoid line while he's still calm");
    let after = world.objects.get_component::<DrChaosState>(&dc).unwrap().pissed_off;
    assert!((before - after) >= 1 && (before - after) <= 5, "drained 1-5: {before} -> {after}");
...
fn the_golem_despawns_after_idle_but_an_attack_refreshes_it() {
    ...
    world.objects.get_component_mut::<DrChaosGolem>(&golem).unwrap().last_attack_tick = world.tick;
    world.tick += 29 * 60 * 10;
    dr_chaos::on_golem_attacked(&mut world, golem);
    dr_chaos::handle_golem_despawn(&mut world, golem);
    assert!(find(&world, CHAOS_GOLEM).is_some(), "the attack refreshed the clock");
...
fn killing_the_golem_arms_a_reset_that_brings_dr_chaos_back() {
    ...
    assert!(world.grand_bosses.get(&CHAOS_GOLEM).unwrap().respawn_time > commons::util::now_millis(), "window set");

    // The reset fires (elapsed) → Dr. Chaos at NORMAL.
    world.grand_bosses.get_mut(&CHAOS_GOLEM).unwrap().respawn_time = 1;
...
fn killing_the_golem_through_the_death_path_marks_it_dead() {
    ...
    assert_eq!(dr_chaos_status(&world), DEAD, "the death hook fired through npc_do_die");
    assert!(world.grand_bosses.get(&CHAOS_GOLEM).unwrap().respawn_time > commons::util::now_millis());
}
...
fn npc_say_text_writes_the_literal_string() {
    ...
    let text: Vec<u8> = "Fools!".encode_utf16().flat_map(|u| u.to_le_bytes()).collect();
    assert!(pkt.windows(text.len()).any(|w| w == text), "the line is on the wire");
}
