...
pub(crate) fn handle_dissolve_clan(world: &mut World, client_id: u32, player_oid: i32) {
    ...
    let Some(clan) = world.clans.get(&clan_id) else { return };
    if clan.ally_id != 0 {
        send_sm(world, client_id, sm_ids::YOU_CANNOT_DISPERSE_THE_CLANS_IN_YOUR_ALLIANCE);
        return;
    }
    if clan_is_at_war(world, clan_id) {
        send_sm(world, client_id, sm_ids::YOU_CANNOT_DISSOLVE_A_CLAN_WHILE_ENGAGED_IN_A_WAR);
        return;
    }
    if clan.castle_id != 0 {
...
pub(crate) fn war_between<'a>(world: &'a World, a: i32, b: i32) -> Option<&'a ClanWar> {
    world.clan_wars.iter().find(|w| (w.attacker_id == a && w.attacked_id == b) || (w.attacker_id == b && w.attacked_id == a))
}
...
fn war_between_mut<'a>(world: &'a mut World, a: i32, b: i32) -> Option<&'a mut ClanWar> {
    world
        .clan_wars
        .iter_mut()
        .find(|w| (w.attacker_id == a && w.attacked_id == b) || (w.attacker_id == b && w.attacked_id == a))
}
...
pub(crate) fn war_relation_bits(world: &World, subject_oid: i32, viewer_oid: i32) -> i32 {
    ...
    let subject_clan = world.objects.get_component::<Player>(&subject_oid).map(|p| p.clan_id).unwrap_or(0);
    let viewer_clan = world.objects.get_component::<Player>(&viewer_oid).map(|p| p.clan_id).unwrap_or(0);
    if subject_clan == 0 || viewer_clan == 0 || subject_clan == viewer_clan {
        return 0;
    }
    let Some(war) = war_between(world, subject_clan, viewer_clan) else { return 0 };
    match war.state {
...
fn send_war_list(world: &World, client_id: u32, clan_id: i32, tab: i32) {
    if let Some(cs) = world.clients.get(&client_id) {
        cs.send(server_packets::pledge_receive_war_list(tab, &war_list_rows(world, clan_id)));
    }
}
...
pub(crate) fn handle_request_start_pledge_war(world: &mut World, client_id: u32, body: &[u8]) {
    let Some(crate::session::ClientSession::InGame(session)) = world.clients.get(&client_id) else { return };
    let player = session.player_object_id();
    let Some(name) = PacketReader::new(body).read_string() else { return };
    let Some(p) = world.objects.get_component::<Player>(&player) else { return };
    let clan_id = p.clan_id;
    let privs = p.clan_privs;
    if clan_id == 0 {
    ...
    if clan.level < 3 || clan.members.len() < CLAN_MEMBERS_FOR_WAR {
        send_sm_with(world, player, sm_ids::CLAN_WAR_NEEDS_LEVEL_3_AND_15_MEMBERS, &[]);
        return;
    }
    ...
    if world.clan_wars.iter().filter(|w| w.involves(clan_id)).count() >= 30 {
        send_sm_with(world, player, sm_ids::CANNOT_DECLARE_WAR_ON_MORE_THAN_30_CLANS, &[]);
        return;
    }
    let Some(target) = world.clans.values().find(|c| c.name.eq_ignore_ascii_case(&name)) else {
        send_sm_with(world, player, sm_ids::CLAN_WAR_TARGET_DOES_NOT_EXIST, &[]);
        return;
    };
    let target_id = target.id;
    if target_id == clan_id {
        send_sm_with(world, player, sm_ids::FOOL_YOU_CANNOT_DECLARE_WAR_AGAINST_YOUR_OWN_CLAN, &[]);
        return;
    }
    let same_ally = {
    ...
    if same_ally {
        send_sm_with(world, player, sm_ids::CANNOT_DECLARE_WAR_ON_ALLIED_CLAN, &[]);
        return;
    }
    ...
    if target.dissolving_expiry_time > now_millis() {
        send_sm_with(world, player, sm_ids::CANNOT_DECLARE_WAR_ON_DISSOLVING_CLAN, &[]);
        return;
    }
        ...
        match war.state_for(clan_id) {
            ...
            ClanWarState::Mutual => {
                send_sm_with(
                    ...
                    player,
                    sm_ids::S1_TEXT,
                    &[SmParam::Text(format!("You have already been at war with {target_name}."))],
                );
                return;
            }
            ClanWarState::BloodDeclaration | ClanWarState::Declaration => {
                ...
                let war = war_between(world, clan_id, target_id).expect("just updated").clone();
                store_war(world, &war);
    ...
    world.scheduler.schedule(
        ...
        crate::scheduler::ScheduledTask::ClanWarTimeout { attacker: clan_id, attacked: target_id },
    );
...
pub(crate) fn handle_request_stop_pledge_war(world: &mut World, client_id: u32, body: &[u8]) {
    ...
    let Some(target) = world.clans.values().find(|c| c.name.eq_ignore_ascii_case(&name)) else {
        send_sm_with(world, player, sm_ids::S1_TEXT, &[SmParam::Text("No such clan.".to_string())]);
        return;
    };
    let target_id = target.id;
    if war_between(world, clan_id, target_id).is_none() {
        send_sm_with(
            ...
            player,
            sm_ids::S1_TEXT,
            &[SmParam::Text("You aren't at war with this clan.".to_string())],
        );
        return;
    }
    let has_priv = world.clans.get(&clan_id).is_some_and(|c| c.has_privilege(player, privs, CL_PLEDGE_WAR));
    if !has_priv {
        send_sm_with(world, player, sm_ids::YOU_ARE_NOT_AUTHORIZED_TO_DO_THAT, &[]);
        return;
    }
    if world.clans.get(&clan_id).map(|c| c.reputation_score).unwrap_or(0) <= 500 {
        send_sm_with(world, player, sm_ids::THE_CLAN_REPUTATION_IS_TOO_LOW, &[]);
        return;
    }
    let member_in_combat =
        online_members(world, clan_id).iter().any(|&oid| super::combat::has_attack_stance(world, oid));
    if member_in_combat {
        send_sm_with(world, player, sm_ids::CEASE_FIRE_CANNOT_BE_CALLED_WHILE_MEMBERS_IN_BATTLE, &[]);
        return;
    }

    add_clan_reputation(world, clan_id, -500);
    let lost = crate::network::enter_world::system_message(sm_ids::YOUR_CLAN_LOST_500_REPUTATION_FOR_WITHDRAWING_FROM_THE_WAR);
...
pub(crate) fn handle_request_surrender_pledge_war(world: &mut World, client_id: u32, body: &[u8]) {
    ...
    let clan_name = world.clans.get(&clan_id).map(|c| c.name.clone()).unwrap_or_default();
    if let Some(cs) = world.clients.get(&client_id) {
        cs.send(server_packets::surrender_pledge_war(&clan_name, &player_name));
    }
    ...
    world
        .scheduler
        .schedule(world.tick + 50, crate::scheduler::ScheduledTask::ClanWarDelete { clan1: clan_id, clan2: target_id });
    broadcast_war_status(world, clan_id, target_id);
}
...
pub(crate) fn delete_clan_wars(world: &mut World, clan1: i32, clan2: i32) {
    world
        .clan_wars
        .retain(|w| !((w.attacker_id == clan1 && w.attacked_id == clan2) || (w.attacker_id == clan2 && w.attacked_id == clan1)));
...
pub(crate) fn handle_clan_war_timeout(world: &mut World, attacker: i32, attacked: i32) {
    let Some(war) = war_between(world, attacker, attacked) else { return };
    if war.state != ClanWarState::BloodDeclaration && war.state != ClanWarState::Declaration {
        return;
    }
    let attacker_name = world.clans.get(&attacker).map(|c| c.name.clone()).unwrap_or_default();
    let attacked_name = world.clans.get(&attacked).map(|c| c.name.clone()).unwrap_or_default();
    ...
    let war = war_between(world, attacker, attacked).expect("just updated").clone();
    store_war(world, &war);
    world
        .scheduler
        .schedule(world.tick + 50, crate::scheduler::ScheduledTask::ClanWarDelete { clan1: attacker, clan2: attacked });
...
pub(crate) fn rearm_clan_wars_at_boot(world: &mut World) {
    let now = now_millis();
    let wars: Vec<(i32, i32, i64, ClanWarState)> =
        world.clan_wars.iter().map(|w| (w.attacker_id, w.attacked_id, w.start_time, w.state)).collect();
    for (attacker, attacked, start, state) in wars {
        ...
        if ended {
            world
                .scheduler
                .schedule(world.tick + 100, crate::scheduler::ScheduledTask::ClanWarDelete { clan1: attacker, clan2: attacked });
        } else if matches!(state, ClanWarState::BloodDeclaration | ClanWarState::Declaration) {
...
pub(crate) fn clan_war_on_kill(world: &mut World, killer_oid: i32, victim_oid: i32) {
    ...
    let Some(war) = war_between(world, killer_clan, victim_clan) else { return };
    ...
    if victim_level > 4 && state == ClanWarState::Mutual {
        ...
        if world.clans.get(&victim_clan).map(|c| c.reputation_score).unwrap_or(0) > 0 {
        ...
        let killer_clan_name = world.clans.get(&killer_clan).map(|c| c.name.clone()).unwrap_or_default();
        let victim_clan_name = world.clans.get(&victim_clan).map(|c| c.name.clone()).unwrap_or_default();
        let down = server_packets::system_message_with(
            sm_ids::BECAUSE_C1_KILLED_BY_S2_CLAN_REPUTATION_DECREASED_BY_1,
            &[SmParam::PlayerName(victim_name), SmParam::Text(killer_clan_name)],
        );
        ...
        let up = server_packets::system_message_with(
            ...
            &[SmParam::Text(victim_clan_name), SmParam::PlayerName(killer_name)],
        );
    ...
    } else if state == ClanWarState::BloodDeclaration && victim_clan == attacker_id && victim_rep >= 0 {
        ...
        if kill_count >= 5 {
            ...
            let progress = server_packets::system_message_with(
                ...
                &[SmParam::Text(victim_clan_name), SmParam::Int(5 - kill_count)],
            );
    ...
    let war = war_between(world, killer_clan, victim_clan).expect("checked above").clone();
    store_war(world, &war);
}
...
use crate::model::clan::{
    ALLY_PENALTY_TYPE_CLAN_DISMISSED, ALLY_PENALTY_TYPE_CLAN_LEAVED, ALLY_PENALTY_TYPE_DISMISS_CLAN,
...
fn store_clan_ally(world: &World, clan_id: i32) {
    let Some(c) = world.clans.get(&clan_id) else { return };
...
fn ally_clan_ids(world: &World, ally_id: i32) -> Vec<i32> {
    ...
    world.clans.values().filter(|c| c.ally_id == ally_id).map(|c| c.id).collect()
}
...
pub(crate) fn handle_create_ally(world: &mut World, client_id: u32, player_oid: i32, args: &str) {
    ...
    let clan_id = p.clan_id;
    if clan_id == 0 || !p.clan_leader {
        send_sm(world, client_id, sm_ids::ONLY_CLAN_LEADERS_MAY_CREATE_ALLIANCES);
        return;
    }
    let Some(clan) = world.clans.get(&clan_id) else { return };
    if clan.ally_id != 0 {
        send_sm(world, client_id, sm_ids::YOU_ALREADY_BELONG_TO_ANOTHER_ALLIANCE);
        return;
    }
    if clan.level < 5 {
        send_sm(world, client_id, sm_ids::TO_CREATE_AN_ALLIANCE_YOUR_CLAN_MUST_BE_LEVEL_5_OR_HIGHER);
        return;
    }
    if clan.ally_penalty_expiry_time > now_millis() && clan.ally_penalty_type == ALLY_PENALTY_TYPE_DISSOLVE_ALLY {
        send_sm(world, client_id, sm_ids::CANNOT_CREATE_A_NEW_ALLIANCE_WITHIN_1_DAY_OF_DISSOLUTION);
        return;
    }
    if clan.dissolving_expiry_time > now_millis() {
        send_sm(world, client_id, sm_ids::SCHEDULED_FOR_CLAN_DISSOLUTION_NO_ALLIANCE_CAN_BE_CREATED);
        return;
    }
    ...
    if name.len() > 16 || name.len() < 2 {
        send_sm(world, client_id, sm_ids::INCORRECT_LENGTH_FOR_AN_ALLIANCE_NAME);
        return;
    }
    if world.clans.values().any(|c| c.ally_name.eq_ignore_ascii_case(name)) {
    ...
    send_sm_with(world, player_oid, sm_ids::S1_TEXT, &[SmParam::Text(format!("Alliance {name} has been created."))]);
}
...
pub(crate) fn handle_dissolve_ally(world: &mut World, client_id: u32, player_oid: i32) {
    ...
    if ally_id == 0 {
        send_sm(world, client_id, sm_ids::YOU_ARE_NOT_CURRENTLY_ALLIED_WITH_ANY_CLANS);
        return;
    }
    if !is_leader || clan_id != ally_id {
        send_sm(world, client_id, sm_ids::THIS_FEATURE_IS_ONLY_AVAILABLE_TO_ALLIANCE_LEADERS);
        return;
    }
    if let Some(pos) = world.objects.get_component::<crate::model::components::Position>(&player_oid) {
        if world.data.zone_data.siege_castle_at(pos.x, pos.y, pos.z).is_some() {
            send_sm(world, client_id, sm_ids::CANNOT_DISSOLVE_ALLIANCE_WHILE_AFFILIATED_CLAN_IN_SIEGE);
            return;
        }
    }

    let dissolved = crate::network::enter_world::system_message(sm_ids::THE_ALLIANCE_HAS_BEEN_DISSOLVED);
...
fn check_ally_join_condition(world: &World, requestor_oid: i32, target_oid: i32) -> bool {
    let Some(rp) = world.objects.get_component::<Player>(&requestor_oid) else { return false };
    let leader_clan_id = rp.clan_id;
    let Some(leader_clan) = world.clans.get(&leader_clan_id) else { return false };
    if leader_clan.ally_id == 0 || !rp.clan_leader || leader_clan_id != leader_clan.ally_id {
        send_sm_with(world, requestor_oid, sm_ids::THIS_FEATURE_IS_ONLY_AVAILABLE_TO_ALLIANCE_LEADERS, &[]);
        return false;
    }
    let now = now_millis();
    if leader_clan.ally_penalty_expiry_time > now && leader_clan.ally_penalty_type == ALLY_PENALTY_TYPE_DISMISS_CLAN {
        send_sm_with(world, requestor_oid, sm_ids::MAY_NOT_ACCEPT_ANY_CLAN_WITHIN_A_DAY_AFTER_EXPELLING, &[]);
        return false;
    }
    ...
    if tp.clan_id == 0 {
        send_sm_with(world, requestor_oid, sm_ids::THE_TARGET_MUST_BE_A_CLAN_MEMBER, &[]);
        return false;
    }
    if !tp.clan_leader {
        send_sm_with(world, requestor_oid, sm_ids::S1_IS_NOT_A_CLAN_LEADER, &[SmParam::Text(tp.name.clone())]);
        return false;
    }
    let Some(target_clan) = world.clans.get(&tp.clan_id) else { return false };
    if target_clan.ally_id != 0 {
        send_sm_with(
            world,
            requestor_oid,
            sm_ids::S1_CLAN_IS_ALREADY_A_MEMBER_OF_S2_ALLIANCE,
            &[SmParam::Text(target_clan.name.clone()), SmParam::Text(target_clan.ally_name.clone())],
        );
        return false;
    }
    if target_clan.ally_penalty_expiry_time > now {
        ...
        if target_clan.ally_penalty_type == ALLY_PENALTY_TYPE_CLAN_DISMISSED {
            send_sm_with(world, requestor_oid, sm_ids::WITHDRAWN_OR_EXPELLED_CLAN_CANNOT_ENTER_ALLIANCE_FOR_A_DAY, &[]);
            return false;
        }
    }
    ...
    let both_in_siege = [requestor_oid, target_oid].iter().all(|&oid| {
        world
            ...
            .is_some_and(|pos| world.data.zone_data.siege_castle_at(pos.x, pos.y, pos.z).is_some())
    });
    if both_in_siege {
        send_sm_with(world, requestor_oid, sm_ids::THE_OPPOSING_CLAN_IS_PARTICIPATING_IN_A_SIEGE_BATTLE, &[]);
        return false;
    }
    if at_war_between(world, leader_clan_id, tp.clan_id) {
        send_sm_with(world, requestor_oid, sm_ids::YOU_MAY_NOT_ALLY_WITH_A_CLAN_YOU_ARE_AT_WAR_WITH, &[]);
        return false;
    }
    if ally_clan_ids(world, leader_clan.ally_id).len() >= MAX_CLANS_IN_ALLY {
        send_sm_with(world, requestor_oid, sm_ids::YOU_HAVE_EXCEEDED_THE_LIMIT, &[]);
        return false;
    }
    true
}
...
pub(crate) fn handle_request_join_ally(world: &mut World, client_id: u32, body: &[u8]) {
    let Some(crate::session::ClientSession::InGame(session)) = world.clients.get(&client_id) else { return };
    let player = session.player_object_id();
    let Some(target_oid) = PacketReader::new(body).read_i32() else { return };
    if client_for_player(world, target_oid).is_none() {
        send_sm_with(world, player, sm_ids::YOU_HAVE_INVITED_THE_WRONG_TARGET, &[]);
        return;
    }
    ...
    let ally_name = world.clans.get(&clan_id).map(|c| c.ally_name.clone()).unwrap_or_default();
    super::party::install_request(
        world,
        player,
        target_oid,
    ...
    if let Some(cs) = client_for_player(world, target_oid).and_then(|cid| world.clients.get(&cid)) {
        cs.send(server_packets::system_message_with(
            ...
            &[SmParam::Text(ally_name), SmParam::Text(player_name(world, player))],
        ));
        cs.send(server_packets::ask_join_ally(player, &player_name(world, player)));
    }
}
...
pub(crate) fn handle_request_answer_join_ally(world: &mut World, client_id: u32, body: &[u8]) {
    ...
    let crate::model::components::RequestKind::AllyInvite { ally_id } = req.kind else { return };
    if !req.answerer {
        return;
    }
    super::party::clear_linked_request(world, player);
    ...
    if answer == 0 {
        send_sm_with(world, player, sm_ids::NO_RESPONSE_YOUR_ENTRANCE_TO_THE_ALLIANCE_HAS_BEEN_CANCELLED, &[]);
        send_sm_with(world, requestor, sm_ids::NO_RESPONSE_INVITATION_TO_JOIN_AN_ALLIANCE_HAS_BEEN_CANCELLED, &[]);
        return;
    }
    // Re-check (the requestor must still lead the same alliance).
    if world.objects.get_component::<Player>(&requestor).map(|p| p.clan_id) != Some(ally_id) {
        return;
    }
    ...
    send_sm_with(world, requestor, sm_ids::SUCCESSFULLY_ADDED_TO_YOUR_FRIEND_LIST, &[]);
...
pub(crate) fn handle_ally_leave(world: &mut World, client_id: u32) {
    ...
    if !p.clan_leader {
        send_sm_with(world, player, sm_ids::ONLY_THE_CLAN_LEADER_MAY_APPLY_FOR_WITHDRAWAL_FROM_THE_ALLIANCE, &[]);
        return;
    }
    let Some(clan) = world.clans.get(&clan_id) else { return };
    if clan.ally_id == 0 {
        send_sm_with(world, player, sm_ids::YOU_ARE_NOT_CURRENTLY_ALLIED_WITH_ANY_CLANS, &[]);
        return;
    }
    ...
    store_clan_ally(world, clan_id);
    refresh_ally_on_members(world, clan_id);
    send_sm_with(world, player, sm_ids::YOU_HAVE_WITHDRAWN_FROM_THE_ALLIANCE, &[]);
}
...
pub(crate) fn handle_ally_dismiss(world: &mut World, client_id: u32, body: &[u8]) {
    ...
    let Some(leader_clan) = world.clans.get(&clan_id) else { return };
    ...
    send_sm_with(world, player, sm_ids::YOU_HAVE_SUCCEEDED_IN_EXPELLING_THE_CLAN, &[]);
}
...
pub(crate) fn handle_request_dismiss_ally(world: &mut World, client_id: u32) {
    let Some(crate::session::ClientSession::InGame(session)) = world.clients.get(&client_id) else { return };
    let player = session.player_object_id();
    let is_leader = world.objects.get_component::<Player>(&player).is_some_and(|p| p.clan_leader);
    if !is_leader {
        send_sm_with(world, player, sm_ids::THIS_FEATURE_IS_ONLY_AVAILABLE_TO_ALLIANCE_LEADERS, &[]);
        return;
    }
...
pub(crate) fn handle_request_ally_info(world: &World, client_id: u32) {
    ...
    let rows: Vec<(String, i32, String, i32, i32)> = clans
        ...
        .filter_map(|&cid| {
            ...
            Some((c.name.clone(), c.level, c.leader_name().to_string(), c.members.len() as i32, online))
        })
        .collect();
    ...
    let (ally_name, leader_clan_name, leader_player_name) = world
        .clans
        .get(&ally_id)
        .map(|c| (c.ally_name.clone(), c.name.clone(), c.leader_name().to_string()))
        .unwrap_or_default();
    let Some(cs) = world.clients.get(&client_id) else { return };
    cs.send(server_packets::alliance_info(&ally_name, total, online, &leader_clan_name, &leader_player_name, &rows));
    cs.send(server_packets::system_message_with(sm_ids::ALLIANCE_INFORMATION, &[]));
    cs.send(server_packets::system_message_with(sm_ids::ALLIANCE_NAME_S1, &[SmParam::Text(ally_name)]));
    cs.send(server_packets::system_message_with(
        sm_ids::ALLIANCE_LEADER_S2_OF_S1,
        &[SmParam::Text(leader_clan_name), SmParam::Text(leader_player_name)],
    ));
    cs.send(server_packets::system_message_with(
    ...
    for (name, level, leader, c_total, c_online) in &rows {
        cs.send(server_packets::system_message_with(sm_ids::CLAN_INFORMATION, &[]));
        cs.send(server_packets::system_message_with(sm_ids::CLAN_NAME_S1, &[SmParam::Text(name.clone())]));
        cs.send(server_packets::system_message_with(sm_ids::CLAN_LEADER_S1, &[SmParam::Text(leader.clone())]));
        cs.send(server_packets::system_message_with(sm_ids::CLAN_LEVEL_S1, &[SmParam::Int(*level)]));
        cs.send(server_packets::system_message_with(
            sm_ids::CONNECTION_S1_TOTAL_S2,
...
use crate::model::clan_entry::{PledgeApplicantInfo, PledgeRecruitInfo, PledgeWaitingInfo, LOCK_TIME_TICKS};

fn is_player_recruit_locked(world: &World, player_id: i32) -> bool {
    world.recruit_player_lock.get(&player_id).is_some_and(|&t| t > world.tick)
}

fn is_clan_recruit_locked(world: &World, clan_id: i32) -> bool {
    world.recruit_clan_lock.get(&clan_id).is_some_and(|&t| t > world.tick)
}
...
fn player_lock_minutes(world: &World, player_id: i32) -> i64 {
    world.recruit_player_lock.get(&player_id).map(|&t| (t.saturating_sub(world.tick) / 600) as i64).unwrap_or(0)
}
fn clan_lock_minutes(world: &World, clan_id: i32) -> i64 {
    world.recruit_clan_lock.get(&clan_id).map(|&t| (t.saturating_sub(world.tick) / 600) as i64).unwrap_or(0)
}
...
fn add_player_application(world: &mut World, clan_id: i32, info: PledgeApplicantInfo) -> bool {
    ...
    world.recruit_applicants.entry(clan_id).or_default().insert(info.player_id, info);
    true
}
...
fn remove_player_application(world: &mut World, clan_id: i32, player_id: i32) -> bool {
    let _ = world.db.send(DbCommand::DeletePledgeApplicant { player_id, clan_id });
    world.recruit_applicants.get_mut(&clan_id).is_some_and(|m| m.remove(&player_id).is_some())
}
...
fn add_to_waiting_list(world: &mut World, info: PledgeWaitingInfo) -> bool {
    ...
    let _ = world.db.send(DbCommand::InsertPledgeWaiting { player_id: info.player_id, karma: info.karma });
...
fn remove_from_waiting_list(world: &mut World, player_id: i32) -> bool {
    ...
    world.recruit_player_lock.insert(player_id, world.tick + LOCK_TIME_TICKS);
    true
}
...
fn remove_from_clan_list(world: &mut World, clan_id: i32) -> bool {
    ...
    world.recruit_clan_lock.insert(clan_id, world.tick + LOCK_TIME_TICKS);
    true
}
...
pub(crate) fn handle_request_pledge_recruit_board_access(world: &mut World, client_id: u32, ex_body: &[u8]) {
    let Some(crate::session::ClientSession::InGame(session)) = world.clients.get(&client_id) else { return };
    let player = session.player_object_id();
    let mut r = PacketReader::new(ex_body);
    let Some(apply_type) = r.read_i32() else { return };
    let Some(karma) = r.read_i32() else { return };
    let Some(information) = r.read_string() else { return };
    let Some(detailed_information) = r.read_string() else { return };
    let Some(application_type) = r.read_i32() else { return };
    let Some(recruit_type) = r.read_i32() else { return };

    let Some(p) = world.objects.get_component::<Player>(&player) else { return };
    let clan_id = p.clan_id;
    let privs = p.clan_privs;
    if clan_id == 0 {
        send_sm_with(world, player, sm_ids::ONLY_THE_CLAN_LEADER_OR_RANK_MANAGER_MAY_REGISTER_THE_CLAN, &[]);
        return;
    }
    if !world.clans.get(&clan_id).is_some_and(|c| c.has_privilege(player, privs, CL_MANAGE_RANKS)) {
        send_sm_with(world, player, sm_ids::ONLY_THE_CLAN_LEADER_OR_RANK_MANAGER_MAY_REGISTER_THE_CLAN, &[]);
        return;
    }
    let info = PledgeRecruitInfo { clan_id, karma, information, detailed_information, application_type, recruit_type };
    match apply_type {
        0 => {
        ...
        1 => {
            if add_to_clan_list(world, clan_id, info) {
                send_sm_with(world, player, sm_ids::ENTRY_APPLICATION_COMPLETE_AUTO_CANCELLED_AFTER_30_DAYS, &[]);
            } else {
                send_sm_with(
                    world,
                    player,
...
/// recruiting clan.
pub(crate) fn handle_request_pledge_recruit_board_detail(world: &World, client_id: u32, ex_body: &[u8]) {
    let Some(clan_id) = PacketReader::new(ex_body).read_i32() else { return };
    let Some(info) = world.recruit_clans.get(&clan_id) else { return };
    if let Some(cs) = world.clients.get(&client_id) {
...
pub(crate) fn handle_request_pledge_recruit_board_search(world: &World, client_id: u32, ex_body: &[u8]) {
    let mut r = PacketReader::new(ex_body);
    let Some(clan_level) = r.read_i32() else { return };
    let Some(karma) = r.read_i32() else { return };
    let Some(search_type) = r.read_i32() else { return };
    let Some(query) = r.read_string() else { return };
    let Some(sort) = r.read_i32() else { return };
    let Some(descending_raw) = r.read_i32() else { return };
    let Some(page) = r.read_i32() else { return };
    let Some(_application_type) = r.read_i32() else { return }; // read, unused (Java: "Helios")
    ...
    let mut matches: Vec<&PledgeRecruitInfo> = if query.is_empty() {
        ...
        world
            ...
            .values()
            .filter(|info| {
                let Some(c) = world.clans.get(&info.clan_id) else { return false };
                if search_type == 1 {
    ...
    if query.is_empty() && !(karma < 0 && clan_level < 0) {
        ...
        matches.sort_by(|a, b| {
            let ord = match sort_by {
                1 => world.clans.get(&a.clan_id).map(|c| c.name.clone()).cmp(&world.clans.get(&b.clan_id).map(|c| c.name.clone())),
                2 => world
                    .clans
                    .get(&a.clan_id)
                    .map(|c| c.leader_name().to_string())
                    .cmp(&world.clans.get(&b.clan_id).map(|c| c.leader_name().to_string())),
                3 => world.clans.get(&a.clan_id).map(|c| c.level).cmp(&world.clans.get(&b.clan_id).map(|c| c.level)),
    ...
    let page_entries: Vec<_> = matches
        ...
        .collect();
    if let Some(cs) = world.clients.get(&client_id) {
        cs.send(server_packets::ex_pledge_recruit_board_search(page, total, &page_entries));
    }
}

/// `RequestPledgeWaitingApply` (ex 0xD7): a clanless player applies to a
/// specific clan.
pub(crate) fn handle_request_pledge_waiting_apply(world: &mut World, client_id: u32, ex_body: &[u8]) {
    let Some(crate::session::ClientSession::InGame(session)) = world.clients.get(&client_id) else { return };
    let player = session.player_object_id();
    let mut r = PacketReader::new(ex_body);
    let Some(karma) = r.read_i32() else { return };
    let Some(clan_id) = r.read_i32() else { return };
    let Some(message) = r.read_string() else { return };
    let Some(p) = world.objects.get_component::<Player>(&player) else { return };
    if p.clan_id != 0 || !world.clans.contains_key(&clan_id) {
        return;
    }
    let info = PledgeApplicantInfo { player_id: player, name: p.name.clone(), level: p.level, karma, clan_id, message };
    if add_player_application(world, clan_id, info) {
        ...
        let leader_id = world.clans.get(&clan_id).map(|c| c.leader_id).unwrap_or(0);
        send_to_member(world, leader_id, server_packets::ex_pledge_waiting_list_alarm());
    } else {
        send_sm_with(
            world,
            player,
...
pub(crate) fn handle_request_pledge_waiting_applied(world: &World, client_id: u32) {
    ...
    let Some(app) = world.recruit_applicants.get(&clan_id).and_then(|m| m.get(&player)) else { return };
    let Some(clan) = world.clans.get(&clan_id) else { return };
...
pub(crate) fn handle_request_pledge_waiting_list(world: &World, client_id: u32, ex_body: &[u8]) {
    ...
    let player = session.player_object_id();
    let Some(clan_id) = PacketReader::new(ex_body).read_i32() else { return };
    if world.objects.get_component::<Player>(&player).map(|p| p.clan_id).unwrap_or(0) != clan_id {
        return;
    }
...
fn send_waiting_list(world: &World, client_id: u32, clan_id: i32) {
    let rows: Vec<_> = world
        .recruit_applicants
        .get(&clan_id)
        .map(|m| m.values().map(|a| (a.player_id, a.name.clone(), 0, a.level)).collect())
        .unwrap_or_default();
    if let Some(cs) = world.clients.get(&client_id) {
...
pub(crate) fn handle_request_pledge_waiting_user(world: &World, client_id: u32, ex_body: &[u8]) {
    ...
    let mut r = PacketReader::new(ex_body);
    let Some(clan_id) = r.read_i32() else { return };
    let Some(player_id) = r.read_i32() else { return };
    if world.objects.get_component::<Player>(&player).map(|p| p.clan_id).unwrap_or(0) != clan_id {
        return;
    }
    match world.recruit_applicants.get(&clan_id).and_then(|m| m.get(&player_id)) {
        Some(app) => {
            if let Some(cs) = world.clients.get(&client_id) {
                cs.send(server_packets::ex_pledge_waiting_user(app.player_id, &app.message));
            }
        }
...
pub(crate) fn handle_request_pledge_waiting_user_accept(world: &mut World, client_id: u32, ex_body: &[u8]) {
    let Some(crate::session::ClientSession::InGame(session)) = world.clients.get(&client_id) else { return };
    let player = session.player_object_id();
    let mut r = PacketReader::new(ex_body);
    let Some(accept) = r.read_i32() else { return };
    let Some(player_id) = r.read_i32() else { return };
    let Some(_clan_id_echo) = r.read_i32() else { return };
    let Some(p) = world.objects.get_component::<Player>(&player) else { return };
    let clan_id = p.clan_id;
    if clan_id == 0 {
        return;
    ...
    if !target_ok {
        let expiry = world.objects.get_component::<Player>(&player_id).map(|t| t.clan_join_expiry_time).unwrap_or(0);
...
/// waiting players.
pub(crate) fn handle_request_pledge_draft_list_search(world: &World, client_id: u32, ex_body: &[u8]) {
    let mut r = PacketReader::new(ex_body);
    let Some(level_min) = r.read_i32() else { return };
    let Some(level_max) = r.read_i32() else { return };
    let Some(_class_id) = r.read_i32() else { return }; // read, TODO: role filter unhandled in Java too
    let Some(query) = r.read_string() else { return };
    let Some(sort) = r.read_i32() else { return };
    let Some(descending_raw) = r.read_i32() else { return };
    let descending = descending_raw == 2;
    ...
    let mut rows: Vec<&PledgeWaitingInfo> = if query.is_empty() {
        world.recruit_waiting.values().filter(|p| p.level >= level_min && p.level <= level_max).collect()
    } else {
        let q = query.to_lowercase();
        world.recruit_waiting.values().filter(|p| p.name.to_lowercase().contains(&q)).collect()
    };
    if query.is_empty() {
        let sort_by = sort.clamp(1, 4);
        rows.sort_by(|a, b| {
    ...
    let out: Vec<_> = rows.iter().map(|p| (p.player_id, p.name.clone(), p.karma, p.class_id, p.level)).collect();
    if let Some(cs) = world.clients.get(&client_id) {
...
pub(crate) fn handle_request_pledge_draft_list_apply(world: &mut World, client_id: u32, ex_body: &[u8]) {
    let Some(crate::session::ClientSession::InGame(session)) = world.clients.get(&client_id) else { return };
    let player = session.player_object_id();
    let mut r = PacketReader::new(ex_body);
    let Some(apply_type) = r.read_i32() else { return };
    ...
    match apply_type {
        0 => {
            if remove_from_waiting_list(world, player) {
                send_sm_with(world, player, sm_ids::ENTRY_APPLICATION_CANCELLED_YOU_MAY_APPLY_AFTER_5_MINUTES, &[]);
            }
        }
        1 => {
            let info = PledgeWaitingInfo { player_id: player, level: p.level, karma, class_id: p.class_id, name: p.name.clone() };
            if add_to_waiting_list(world, info) {
                send_sm_with(world, player, sm_ids::ENTERED_INTO_WAITING_LIST_AUTO_DELETED_AFTER_30_DAYS, &[]);
            } else {
                send_sm_with(
                    world,
                    player,
...
pub(crate) fn handle_request_pledge_sign_in_for_open_joining_method(world: &mut World, client_id: u32, ex_body: &[u8]) {
    let Some(crate::session::ClientSession::InGame(session)) = world.clients.get(&client_id) else { return };
    let player = session.player_object_id();
    let Some(clan_id) = PacketReader::new(ex_body).read_i32() else { return };
    let Some(recruit) = world.recruit_clans.get(&clan_id) else { return };
    let _ = recruit;
    let Some(p) = world.objects.get_component::<Player>(&player) else { return };
    if p.clan_id != 0 {
        return;
