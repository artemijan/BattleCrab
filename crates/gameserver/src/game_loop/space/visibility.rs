...
fn char_info_bytes(world: &World, player_id: i32) -> Option<bytes::Bytes> {
    ...
    let cubics = world
        .objects
        .get_component::<crate::game_loop::skills::cubic::Cubics>(&player_id)
        .map(|c| c.ids())
        .unwrap_or_default();
    Some(bytes::Bytes::from(server_packets::char_info(
        &v,
        &crate::game_loop::abnormal::visual_effects(world, player_id),
        &cubics,
        &crate::game_loop::character::player_info::char_info_state(world, player_id),
    )))
}
...
pub(crate) fn npc_info_bytes(world: &World, npc_id: i32) -> Option<bytes::Bytes> {
    ...
    let visuals = crate::game_loop::abnormal::visual_effects(world, npc_id);
...
pub(crate) fn on_enter_world(world: &World, client_id: u32, object_id: i32) {
    ...
    let my_instance = crate::game_loop::helpers::instance_of(world, object_id);
    for cs in world.clients.values() {
        if let ClientSession::InGame(s) = cs {
            ...
            if regions_adjacent(my_region, other_region)
                && crate::game_loop::helpers::instance_of(world, object_id)
                    == crate::game_loop::helpers::instance_of(world, other_id)
            {
    ...
    for &trader_id in world.offline_traders.keys() {
        ...
        if regions_adjacent(my_region, trader_region)
            && crate::game_loop::helpers::instance_of(world, trader_id) == my_instance
        {
    ...
    for npc_id in world.npcs_visible_from(my_region) {
        ...
        if crate::game_loop::helpers::instance_of(world, npc_id) != my_instance {
            continue;
        }
...
pub(crate) fn update_region(world: &mut World, object_id: i32) {
        ...
        .collect::<Vec<_>>()
    {
        ...
        let same_instance = crate::game_loop::helpers::instance_of(world, object_id)
            == crate::game_loop::helpers::instance_of(world, other_id);
    ...
    for npc_oid in departed {
        crate::game_loop::npc::ai::on_forget_object(world, npc_oid, object_id);
    }
    ...
    if let Some(cs) = my_client.and_then(|cid| world.clients.get(&cid)) {
        for npc_id in world.npcs_visible_from(new) {
            ...
            if !regions_adjacent(old, npc_region.0)
                && crate::game_loop::helpers::instance_of(world, npc_id) == my_instance
        ...
        for door_id in world.doors_visible_from(new) {
            ...
            if !regions_adjacent(old, door_region.0)
                && crate::game_loop::helpers::instance_of(world, door_id) == my_instance
            {
        ...
        for so_id in world.statics_visible_from(new) {
            ...
            if !regions_adjacent(old, so_region.0)
                && crate::game_loop::helpers::instance_of(world, so_id) == my_instance
            {
        ...
        for item_id in world.ground_items_visible_from(new) {
            ...
            if !regions_adjacent(old, r.0)
                && crate::game_loop::helpers::instance_of(world, item_id) == my_instance
...
pub(crate) fn update_npc_region(world: &mut World, npc_object_id: i32) {
    ...
    let npc_instance = crate::game_loop::helpers::instance_of(world, npc_object_id);
        ...
        .chain(world.in_game_players_visible_from(new))
    {
        ...
        let same_instance =
            crate::game_loop::helpers::instance_of(world, player_id) == npc_instance;
...
pub(crate) fn movement_tick(world: &mut World) {
    ...
    for id in &outcome.moved_players {
        ...
        crate::game_loop::space::zones::revalidate_zone(world, *id, false);
    }
    ...
    for id in &outcome.arrived {
        ...
        crate::game_loop::npc::ai::on_npc_arrived(world, *id);
    }
