...
pub(crate) fn run_regen_tick(world: &mut World) {
    ...
    for (client_id, object_id) in targets {
        ...
        crate::game_loop::stats::passive_skills::refresh_on_hp_change(world, object_id);
...
pub(crate) fn clan_hall_regen_mult(world: &World, object_id: i32) -> (f64, f64) {
    ...
    let hp =
        crate::game_loop::clans::hall_function::active_function_value(world, hall_id, "HP_REGEN")
            .unwrap_or(1.0);
    let mp =
        crate::game_loop::clans::hall_function::active_function_value(world, hall_id, "MP_REGEN")
            .unwrap_or(1.0);
    (hp, mp)
}
...
pub(crate) fn castle_regen_mult(world: &World, object_id: i32) -> (f64, f64) {
    ...
    let mult = |func_type: i32| {
        crate::game_loop::siege::treasury::castle_function(world, castle_id, func_type)
