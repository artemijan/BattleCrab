...
fn resolve(world: &World, param: &str) -> Option<usize> {
    if param.chars().all(|c| c.is_ascii_digit()) {
        ...
        world
            .castles
            .iter()
            .position(|c| c.name.eq_ignore_ascii_case(param))
    }
}
...
pub(super) fn admin_castlemanage(
    world: &mut World,
    client_id: u32,
    gm_object_id: i32,
    args: &[&str],
...
fn show_castle_menu(world: &mut World, client_id: u32, idx: usize) {
    ...
    let (owner_name, owner_clan) =
        match owner_clan(world, castle_id).and_then(|cid| world.clans.get(&cid)) {
...
fn switch_side(world: &mut World, client_id: u32, idx: usize) {
    ...
    match next {
        Some(side) => {
            ...
            let _ = world.db.send(DbCommand::UpdateCastleSide {
                castle_id,
