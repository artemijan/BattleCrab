...
fn add_exp_sp(world: &mut World, client_id: u32, object_id: i32, args: &[&str]) -> Guard<()> {
    ...
    match (
        ...
        _ => helpers::send_message(world, client_id, "Usage: //add_exp_sp exp sp"),
    }
...
fn remove_exp_sp(world: &mut World, client_id: u32, object_id: i32, args: &[&str]) -> Guard<()> {
    ...
    match (
        ...
        (2, Some(_), Some(_)) => {}
        _ => helpers::send_message(world, client_id, "Usage: //remove_exp_sp exp sp"),
    }
    admin_add_exp_sp_menu(world, client_id, object_id);
    Ok(())
}
...
pub(super) fn admin_set_color(
    ...
    let Some(color) = args
        ...
        helpers::send_message(world, client_id, "Usage: //setcolor <hex, e.g. FF0000>");
        return;
    };
    let target = target_player(world, object_id);
    if let Some(p) = world.objects.get_component_mut::<Player>(&target) {
...
pub(super) fn admin_set_enchant(
    ...
    let Some(value) = helpers::nth_arg::<i32>(args, 0).filter(|v| (0..=127).contains(v)) else {
...
pub(super) fn admin_setsubclass(world: &mut World, client_id: u32, object_id: i32, args: &[&str]) {
    ...
    let Some(class_id) = helpers::nth_arg::<i32>(args, 0) else {
        let listing = world
            ...
            .unwrap_or_default();
        helpers::send_message(world, client_id, &listing);
        return;
    };

    match add_subclass(world, target, class_id) {
        Ok(index) => helpers::send_message(
            world,
            client_id,
            &format!("Added subclass {class_id} in slot {index}."),
        ),
        Err(AddError::SlotsFull) => {
            helpers::send_message(world, client_id, "No free subclass slots.")
        }
        Err(AddError::AlreadyHave) => {
            helpers::send_message(world, client_id, "That class is already held.")
        }
        Err(AddError::UnknownClass) => helpers::send_message(
            world,
            client_id,
            &format!("Class id {class_id} does not exist."),
        ),
...
pub(super) fn admin_changesubclass(
    ...
    let Some(index) = helpers::nth_arg::<i32>(args, 0) else {
        helpers::send_message(
            world,
            client_id,
