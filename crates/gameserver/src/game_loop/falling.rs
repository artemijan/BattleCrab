...
use super::helpers::{is_dead, send_sm_to_player, send_to_player};
...
/// [`handle_validate_position`](super::position::handle_validate_position)
...
pub(crate) fn falling_damage_tick(world: &mut World) {
    ...
    for (oid, damage) in due {
        ...
        super::combat::player_receive_damage_ex(world, oid, oid, applied, true);
        send_sm_to_player(
            world,
            oid,
