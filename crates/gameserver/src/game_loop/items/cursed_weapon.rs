...
//! [`crate::game_loop::admin::cursed_weapons`], where the `//cw_*` GM commands drove it
...
pub(crate) fn increase_kills(world: &mut World, idx: usize) {
    ...
    crate::game_loop::admin::cursed_weapons::save_data(world, idx);
...
fn drop_from_wielder(world: &mut World, idx: usize, victim_oid: i32, killer_oid: i32) {
    ...
    crate::game_loop::skills::remove_player_skill(world, victim_oid, skill_id);
    ...
    if let Some(cid) = crate::game_loop::helpers::client_for_player(world, victim_oid) {
        ...
        crate::game_loop::items::refresh_equip_state(world, cid, victim_oid);
    }
    crate::game_loop::character::player_info::broadcast_user_info(world, victim_oid);
...
pub(crate) fn do_transform(world: &mut World, target: i32, item_id: i32) {
    ...
    {
        crate::game_loop::admin::transforms::remove_transform(world, target);
    }
    crate::game_loop::admin::transforms::apply_transform(world, target, transform_id);
}
...
pub(crate) fn give_skill(world: &mut World, idx: usize, target: i32) {
    ...
    crate::game_loop::stats::passive_skills::refresh_conditioned_passives(world, target);
    crate::game_loop::admin::refresh_skill_list(world, target);
}
...
fn destroy_stray_cursed_items(world: &mut World, client_id: u32, object_id: i32) {
    let stale_skills: Vec<i32> = (0..world.cursed_weapons.len())
        .flat_map(|idx| {
            let item_id = world.cursed_weapons[idx].item_id;
            crate::game_loop::admin::cursed_weapons::curse_granted_skill_ids(world, idx, item_id)
        })
        .filter(|id| {
            world
                .objects
    ...
    if !stale_skills.is_empty() {
        for skill_id in stale_skills {
            crate::game_loop::skills::remove_player_skill(world, object_id, skill_id);
        }
        crate::game_loop::admin::refresh_skill_list(world, object_id);
