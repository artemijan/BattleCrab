...
pub(crate) fn broadcast_char_info_now(world: &mut World, object_id: i32) {
    ...
    let cubics = world
        .objects
        .get_component::<crate::game_loop::skills::cubic::Cubics>(&object_id)
        .map(|c| c.ids())
        .unwrap_or_default();
    ...
    let char_info = server_packets::char_info(
        &v,
        &crate::game_loop::abnormal::visual_effects(world, object_id),
        &cubics,
...
pub(crate) fn char_info_state(world: &World, object_id: i32) -> server_packets::CharInfoState {
    ...
    server_packets::CharInfoState {
        in_combat: crate::game_loop::combat::has_attack_stance(world, object_id),
