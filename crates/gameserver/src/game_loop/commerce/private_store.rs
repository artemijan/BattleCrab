...
pub(crate) fn handle_buy(world: &mut World, client_id: u32, body: &[u8]) {
    ...
    if crate::game_loop::items::cursed_weapon::is_cursed(world, buyer) {
        return;
    }
...
pub(crate) fn handle_store_sell(world: &mut World, client_id: u32, body: &[u8]) {
    ...
    if crate::game_loop::items::cursed_weapon::is_cursed(world, seller) {
        return;
    }
    ...
    if !crate::game_loop::combat::target::can_interact(world, seller, owner) {
        return;
    }
