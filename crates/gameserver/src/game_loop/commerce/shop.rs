...
fn restock_ticks(delay_ms: i64) -> u64 {
    (delay_ms.max(0) as u64 * crate::game_loop::time::TICKS_PER_SECOND).div_ceil(1000)
}
...
pub(crate) fn handle_request_buy_item(world: &mut World, client_id: u32, body: &[u8]) {
    ...
    if !helpers::is_gm(world, player) {
        ...
        if weight < 0 || !crate::game_loop::stats::weight::validate_weight(world, player, weight) {
            send_sm_and_action_failed(
                world,
                client_id,
        ...
        if !crate::game_loop::stats::weight::validate_capacity(world, player, slots) {
...
pub(crate) fn handle_request_preview_item(world: &mut World, client_id: u32, body: &[u8]) {
    ...
    let delay =
        (world.cfg.general.wear_delay.max(0) as u64) * crate::game_loop::time::TICKS_PER_SECOND;
    world.scheduler.schedule(
        world.tick + delay,
