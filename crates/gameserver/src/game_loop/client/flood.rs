...
//! [`crate::game_loop::client::dispatch::on_packet`] consults it once, before the handler runs.
...
/// (the same grep blind spot `game_loop::social::chat`'s module header describes).
#[cfg(test)]
const UNMAPPED_ACTIONS: [FloodAction; 5] = [
    ...
    // `ChatHeroVoice` (`game_loop::social::chat::hero_voice` calls `gate` directly).
