...
pub(crate) fn handle_voiced(
    ...
    if world.cfg.auto_play.premium_only
        && !crate::game_loop::admin::premium::has_premium_status(world, player_oid)
    {
        return;
    }
    if command != "play" {
        // The three sub-panels are auto-use's.
        super::use_items::handle_voiced(world, client_id, player_oid, command, args);
        return;
    }
...
fn move_to(world: &mut World, player_oid: i32, from: Position, dest: (i32, i32, i32)) {
    if let Some(cid) = client_for_player(world, player_oid) {
        crate::game_loop::space::position::start_move(world, cid, player_oid, from, dest, None);
    }
}
...
fn set_target(world: &mut World, player_oid: i32, target: i32) {
    if let Some(cid) = client_for_player(world, player_oid) {
        crate::game_loop::combat::target::set_target(world, cid, player_oid, Some(target));
    }
}

fn clear_target(world: &mut World, player_oid: i32) {
    if let Some(cid) = client_for_player(world, player_oid) {
        crate::game_loop::combat::target::set_target(world, cid, player_oid, None);
    }
}

fn attack(world: &mut World, player_oid: i32, target: i32) {
    if let Some(cid) = client_for_player(world, player_oid) {
        crate::game_loop::combat::start_attack_intent(world, cid, player_oid, target);
    }
}
