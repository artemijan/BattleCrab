            ...
            crate::game_loop::party::command_channel::cc_id_of_party(world, party_id)
    {
        ...
        if is_cc_leader {
            if world.matching_rooms.room_id_of(player).is_none() {
                crate::game_loop::party::command_channel::create_cc_room(world, player, party_id);
            }
        } else {
            send_sm(
                    world,
...
fn add_member(world: &mut World, room_id: i32, player: i32) -> bool {
    ...
    {
        return crate::game_loop::party::command_channel::cc_room_add_member(
            world, room_id, player,
        );
    }
...
fn remove_member(world: &mut World, room_id: i32, player: i32, kicked: bool) {
    if world
        ...
        .is_some_and(|r| r.kind == RoomKind::CommandChannel)
    {
        crate::game_loop::party::command_channel::cc_room_remove_member(
            world, room_id, player, kicked,
        );
        return;
    }
...
pub(crate) fn handle_ask_join_party_room(world: &mut World, client_id: u32, body: &[u8]) {
    ...
    let Some((_, target)) = crate::game_loop::party::find_player_by_name(world, &name) else {
...
pub(crate) fn handle_answer_join_party_room(world: &mut World, client_id: u32, body: &[u8]) {
    ...
    let Some(req) = crate::game_loop::party::clear_linked_request(world, player) else {
        send_sm(world, player, sm_ids::THAT_PLAYER_IS_NOT_ONLINE, &[]);
        return;
    };
