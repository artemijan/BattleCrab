...
fn busy(world: &World, oid: i32) -> bool {
    world.objects.has_component::<Trade>(&oid)
        || world.objects.get_component::<crate::model::Player>(&oid).map(|p| p.store_type).unwrap_or(0) != 0
}

/// `TradeRequest` (0x1A): ask the targeted player to trade.
pub(crate) fn handle_request(world: &mut World, client_id: u32, body: &[u8]) {
    let Some(from) = player_of(world, client_id) else { return };
    let Some(target) = PacketReader::new(body).read_i32() else { return };
    if target == from || !world.objects.has_component::<crate::model::Player>(&target) || busy(world, target) || busy(world, from) {
...
pub(crate) fn handle_answer(world: &mut World, client_id: u32, body: &[u8]) {
    let Some(me) = player_of(world, client_id) else { return };
    let response = PacketReader::new(body).read_i32().unwrap_or(0);
    let requester = world.objects.get_component::<PendingTrade>(&me).map(|p| p.from);
    world.objects.remove_component::<PendingTrade>(&me);
    let Some(requester) = requester else { return };
    if response != 1 || busy(world, me) || busy(world, requester) || !world.objects.has_component::<crate::model::Player>(&requester) {
        return;
    }
    // Open the trade on both sides.
    world.objects.add_components(&me, Trade { partner: requester, items: Vec::new(), confirmed: false });
    world.objects.add_components(&requester, Trade { partner: me, items: Vec::new(), confirmed: false });
...
fn open_window(world: &World, viewer: i32, partner: i32) {
    let level = world.objects.get_component::<crate::model::Player>(&partner).map(|p| p.level as u8).unwrap_or(1);
...
pub(crate) fn handle_add_item(world: &mut World, client_id: u32, body: &[u8]) {
    let Some(pkt) = cp::AddTradeItem::read(body) else { return };
    let Some(me) = player_of(world, client_id) else { return };
    let Some(partner) = world.objects.get_component::<Trade>(&me).map(|t| t.partner) else { return };
    // Validate the instance: in inventory, not equipped/quest, count in range,
    // and not the full amount already offered.
    let Some((item_id, held, enchant)) = world.objects.get_component::<Inventory>(&me).and_then(|inv| {
        (inv.paperdoll_slot_of(pkt.object_id).is_none())
            .then(|| inv.items().iter().find(|it| it.object_id == pkt.object_id).map(|it| (it.item_id, it.count, it.enchant_level)))
            .flatten()
    }) else {
        return;
    };
    ...
    let already = world.objects.get_component::<Trade>(&me).map(|t| t.items.iter().filter(|s| s.object_id == pkt.object_id).map(|s| s.count).sum::<i64>()).unwrap_or(0);
    ...
    if let Some(t) = world.objects.get_component_mut::<Trade>(&me) {
        match t.items.iter_mut().find(|s| s.object_id == pkt.object_id) {
            Some(s) => s.count += add,
            None => t.items.push(StoreItem { object_id: pkt.object_id, item_id, count: add, price: 0, enchant }),
        }
    }
...
pub(crate) fn handle_done(world: &mut World, client_id: u32, body: &[u8]) {
    ...
    let both = world.objects.get_component::<Trade>(&me).is_some_and(|t| t.confirmed)
        && world.objects.get_component::<Trade>(&partner).is_some_and(|t| t.confirmed);
    if both {
...
fn transfer_side(world: &mut World, from: i32, to: i32) {
    let offered = world.objects.get_component::<Trade>(&from).map(|t| t.items.clone()).unwrap_or_default();
    for line in offered {
        // Clamp to what's actually still held.
        let held = world.objects.get_component::<Inventory>(&from).and_then(|inv| inv.items().iter().find(|it| it.object_id == line.object_id).map(|it| it.count)).unwrap_or(0);
        ...
        if let Some(new_oid) = world.alloc_object_id() {
            if let Some(inv) = world.objects.get_component_mut::<Inventory>(&to) {
                inv.insert_instance(&world.data.item_data, new_oid, line.item_id, n, line.enchant);
            }
        }
    }
}
