...
// (`auto_play::TICK_PERIOD`) rather than keeping a second constant in step.
...
pub(crate) fn tick(world: &mut World) {
    ...
    let active: Vec<i32> = world
        .in_game_player_oids()
        .filter(|oid| {
            super::auto_play::settings(world, *oid).is_some_and(|s: AutoPlaySettings| s.active)
        })
        .collect();
...
fn use_potion(world: &mut World, player_oid: i32) {
    let percent = super::auto_play::settings(world, player_oid)
...
fn cast(world: &mut World, player_oid: i32, skill_id: i32) {
    if let Some(cid) = client_for_player(world, player_oid) {
        super::skills::cast::use_magic(world, cid, player_oid, skill_id, false, false);
    }
}
