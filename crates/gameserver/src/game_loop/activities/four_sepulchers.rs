...
pub(crate) fn restore_entry_times(world: &mut World) {
    for sepulcher in 1..=4 {
        let key = crate::game_loop::upkeep::global_vars::four_sepulchers_key(manager_npc_id_of(
            sepulcher,
        ));
        let stamp = crate::game_loop::upkeep::global_vars::get_i64(world, &key, 0);
...
pub(crate) fn try_enter(world: &mut World, manager_oid: i32, player: i32) -> EnterOutcome {
    ...
    crate::game_loop::upkeep::global_vars::set(
        world,
        &crate::game_loop::upkeep::global_vars::four_sepulchers_key(manager_npc_id_of(sepulcher)),
        now,
    );
