...
/// [`reco`](super::reco) pattern) makes the older schedule stale instead, so a
...
fn reward_period_ticks(world: &World) -> u64 {
    (world.cfg.premium.pc_cafe_reward_time.max(1) as u64)
        .div_ceil(100)
        .max(1)
}
...
pub(crate) fn give_retail_point(world: &mut World, player_object_id: i32) {
    ...
    if cfg.pc_cafe_only_premium
        && !super::admin::premium::has_premium_status(world, player_object_id)
    {
        return;
    }
...
pub(crate) fn give_point(world: &mut World, player_object_id: i32, exp: f64) {
    ...
    let in_zone = |kind: crate::data::zone_data::ZoneKind| {
        world
            .objects
