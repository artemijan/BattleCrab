...
fn recv(rx: &std::sync::mpsc::Receiver<gameserver::events::GameEvent>) -> DbEvent {
    loop {
        let gameserver::events::GameEvent::Db(event) =
        ...
        match event {
            ...
            | DbEvent::OfflineTradersLoaded { .. } => continue,
            other => return other,
        }
    }
}
