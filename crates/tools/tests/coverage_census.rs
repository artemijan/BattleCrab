...
///
/// Every milestone of the port landed, and each one still shipped a handful of
/// deliberately-deferred behaviours marked at the site. That is the intended end
/// state, not a contradiction — but prose about "what is left" is the least
/// reliable artefact in this repo: on 2026-08-03 two separate documents were
/// found claiming work was outstanding that had been done for milestones, and
/// five `TODO` markers one of them said existed were absent from the code
/// entirely.
///
...
fn deferral_markers_match_the_recorded_inventory() {
    ...
    // there is no prose half of it to drift out of sync, deliberately. A gap
    // recorded here should also be described at the site that defers it.
    // (`docs/DEFERRALS.md` was the human-readable half until 2026-08-07, when
    // the last marker closed and the file was deleted; it is recoverable with
    // `git log --diff-filter=D -- docs/DEFERRALS.md`.)
    //
    ...
    assert_eq!(
        ...
        "the TODO(G<N>) deferral inventory moved. If you recorded a new gap, add it \
         here and describe it at the site that defers it; if you closed \
         one, take it off. A gap that is not in this list is invisible."
    );
}
