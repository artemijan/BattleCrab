...
import static org.l2jmobius.gameserver.network.SystemMessageId.YOU_DO_NOT_HAVE_ENOUGH_REQUIRED_ITEMS;
...
public class LastImperialTomb extends AbstractInstance {
  ...
  private static final int FRINTEZA_SCROLL= 8073;
  ...
  public String onTalk(Npc npc, Player player) {
    if (npc.getId() == GUIDE) {
      if (player.getInventory().getInventoryItemCount(FRINTEZA_SCROLL, -1) > 0) {
        enterInstance(player, npc, TEMPLATE_ID);
      } else {
        SystemMessage message = new SystemMessage(YOU_DO_NOT_HAVE_ENOUGH_REQUIRED_ITEMS);
