...
| Game  | G34 Skills, effects & abnormal-state parity                 | 🚧 **S0 (coverage harness) + S1 (condition engine) + S2 (BasicPropertyResist) + S3 (EffectFlag breadth) landed; S4 (effect sweep) in progress** (plan: [PLAN_G34_SKILL_PARITY.md](PLAN_G34_SKILL_PARITY.md)) — the skill parser is *fail-open*: an unrecognised `<effect name>` yields no `SkillEffect`, an effect list that ends up empty is dropped by `apply_skill_effects`' empty-effects guard, and a `<condition>` it doesn't recognise is never enforced, so a skill can cast, animate, burn MP and reuse and do nothing — or fire where Java refuses it. `SkillGaps` now records every drop at the fallback arm itself (7 categories: effect name, unbuilt effect scope, condition in each of Java's three `SkillConditionScope` blocks, and the `targetType`/`affectScope`/`affectObject`/`operateType` catch-alls), `log_gaps` warns per category at boot, and `datapack_skill_coverage_census` asserts the exact learnable-source name lists plus the totals against a text-scan of the datapack's own reachability. Baseline: **216** unhandled effect names (54 learnable-sourced), **111** unported condition `block/name` pairs, and **275 of 758 learnable skills (36 %)** that drop an effect or ignore a condition. Sabotage-verified twice. **S1** then landed the condition engine: `<conditions>`/`<targetConditions>`/`<passiveConditions>` parse into a `Vec<SkillCondition>` per Java `SkillConditionScope` (the latter two were never even entered before), and `skills::conditions::check_cast` evaluates GENERAL then TARGET from `use_magic_on` — after target resolution, where `Player.useMagic` calls `checkCondition`. 28 condition kinds cover every one with a learnable source (`EquipWeapon` alone gates 88 skills); Java's two-message refusal, the GM bypass and the mounted-and-bad gate come with it; the inline `OpExistNpc` and `CanTransform` gates were folded in, leaving one representation each. Condition axis: 111 pairs / 215 learnable skills → **69 / 1** (`OpSweeper`, deliberate), headline **275 → 79**. **S2** ported the mesmerizing-debuff resistance chain: `<basicProperty>` parses (390 learnable skills), and `game_loop::basic_property` implements both terms `calcEffectSuccess` reads off it — the `ABNORMAL_RESIST_*` **stat** subtracted inside `baseMod`, and the **accrual chain** (1.0/0.6/0.3/0 over a 15 s window) multiplied *after* the clamp, which is what makes level 3 a hard immunity rather than a rate the 10 floor rescues. Accrual is on the landed path only; mobs accrue and players don't (`SIXTH_CLASS_GROUP` is empty here), so PvE gains stun-lock resistance and PvP is untouched. Effect names 216 → 214. **S3** audited all 23 missing `EffectFlag`s and ported the nine that are flag-only, each with its single Java gate (`BuffBlock`, `PhysicalShieldAngleAll`, `Passive`, `Untargetable`, `DisableTargeting`, `PhysicalAttackMute`, `BlockResurrection`, `BlockEscape`, `AbnormalShield` — the last dead in Java too, nothing reads it). Five more are held for S4 (their effects have mechanics beyond the bit) and nine have no source on this dist. Also folded `isStayAfterDeath()`'s three tags (`stayAfterDeath || irreplacableBuff || isNecessaryToggle`), which had been stripping 30 learnable transform buffs on death. Effect names 214 → 205, headline 79 → 72. **S4** is the largest remaining piece and cannot be done in bulk: of the 49 effect names with a learnable source, 14 are plain `AbstractStat*Effect` but **none of their `Stat`s exists in this port**, so each needs a variant, a registry entry *and* a consumer — a registry line alone shrinks the census while the effect stays inert. Sub-slice 1 took the three whose consumers already existed: `Breath` (21 skills grant it, against a comment claiming none did), `WeightLimit` (×4 capacity from Weight Limit 150, previously ignored) and `WeightPenalty` (whose name says *band* but whose every Java caller subtracts it from carried weight). Sub-slice 2 took the four skill-damage multipliers (`PhysicalSkillPower`/`MagicalSkillPower` — the latter lives inside `calcMagicDam`, so `HpDrain` gets it too — and the physical-skill crit pair, whose branch was a literal `2.0`). Sub-slice 3 took the aggro family — `HateAttack` (auto-attacks only, Java's `skill == null` branch), `TargetMe` + `TargetMeProbability` (playables only, and `TargetMe` also *locks* the target so `Npc.canTarget` refuses any other NPC). Sub-slice 4 took the mitigation/counter family (`AreaDamage` — negative on this dist, so mitigation; `TransferDamageToSummon`; `CounterPhysicalSkill`, a *chance* and melee-only; `SkillEvasion`, a per-magicType map; `SkillTurning`, an offensive cast-break). Sub-slice 5 took `EnlargeAbnormalSlot` (the good-buff slot cap, modelled as a derived stat so it cannot drift) and `DispelBySlotMyself` (which spares an `irreplacableBuff`). Sub-slice 6 took `SkillMastery`/`SkillMasteryRate` (the cooldown-collapse proc, whose stat stores a **`BaseStat` ordinal** — and Java's enum order differs from the port's, so it is parsed by name) and `Lucky` (an empty effect in Java; the buff's presence is the mechanic). Sub-slice 7 took `CriticalRatePositionBonus` (Focus Chance 356 — the only skill declaring all three positions, so it *punishes* attacking from the front), `MpVampiricAttack` (whose config gate is shaped opposite to the HP twin's: skills by default, not melee) and `CubicMastery` (whose `MAX_CUBIC` nothing in Java reads — the second dead stat found). Sub-slice 8 took the heal ceiling: `LimitHp`/`LimitCp` (`MAX_RECOVERABLE_*` — Noblesse Harmony/Symphony grant them *negative*, capping heals at 70 % HP / 60 % CP, and the port clamped to the raw pool, identical until someone casts them), plus `CpHealPercent` and `HpByLevel` (which heals the **caster**, not the target). Sub-slice 9 took the three bespoke ones: `Bluff` (the Backstab set-up spin — needing two packets the port lacked, `StartRotating` 0x7A / `StopRotating` 0x61 — with Java's raid exemption), `Unsummon`, and `DeathLink` (power scaled by the caster's *missing* HP, so at full health it lands, costs MP and does **nothing**). Sub-slice 10 took the whole of Unlock (27): the `DOOR_TREASURE` target type (the last learnable entry on the target axis), `OpenDoor` (two distinct refusals — a non-`BY_SKILL` door cannot be picked at all, a missed roll can be retried) and `OpenChest` (a level *band*, whose other half lands on the death side: an unlocked box pays no exp/sp and keeps its own drop list, a smashed one rolls a different npc id's). Two dist findings came with it — **no `type="Chest"` NPC is spawned anywhere on this datapack**, and the smashed-chest remap points at ids that do not exist here (Java would NPE; the port falls back to the chest's own list). Sub-slice 11 took the sustain family: `ChameleonRest` (Relax minus its HP-full stop, plus `SILENT_MOVE` — you rest to hide, not to heal), `ManaHealOverTime` (the mirror of the MP drain, with Java's asymmetric guards and its floor of 1) and `RebalanceHP` (Balance Life, a *redistribution* — the party's total HP is conserved, so the healthy pay for the dying, and an unpartied cast is simply wasted). Sub-slice 12 took the whole PvP/PvE balance family in one go — fifteen names sharing one consumer, `calculatePvpPveBonus`, which had to be written first: it is a term in every damage formula and the port hard-coded it to 1.0 behind three comments saying the pvp/pve mods *were* 1.0 (true only while nothing granted the stats; ~1300 dist effects do). The shape is a **difference of multipliers** — `1 + (attackMul − defenceMul)` — and the branch is picked by pairing *and* delivery. The PvE level-difference penalty came with it (`SkillDmgPenaltyForLvLDifferences`, ×0.25 at the bottom on this dist, never parsed before). Sub-slice 13 took the physical-attack pair: `PhysicalAttackHpLink` (Fatal Counter's missing-HP scaling, sharing `PhysicalAttack`'s arm) and `PolearmSingleTarget` — Focus Attack's *cost*, missing while both its bonuses landed, so the toggle had been a free upgrade. Sub-slice 14 took the trigger pair: `TriggerSkillByDamage` (Mirage, fired on damage *received* — so it hangs off `apply_attack_damage` and sees skill damage too) and `TriggerSkillByMagicType` (Dance of Shadows, fired when the bearer finishes a cast). Both scan the bearer's **buffs** rather than their skill book, since Java attaches the listener to the effect instance. Sub-slice 15 took servitors and recall: `Betray` (the servitor turns on its owner, stops obeying, and goes auto-attackable via `SummonInfo` bit 0x01), `ImmobilePetBuff` (the `IMMOBILIZED` root, returned at expiry) and `CallParty` (Chant of Gate — a recall with **no** `ConfirmDlg`, gated per member by `checkSummonTargetStatus`). Sub-slice 16 took the death pair: `ReduceDropPenalty` (the exp-loss reduction, keyed on **what killed you** — and its `REDUCE_DEATH_PENALTY_BY_*` twins are the epic's fourth dead stat, read by nothing in Java, so Noblesse Fortune does nothing at all here) and `ResurrectionSpecial` (the auto-resurrect, whose mechanic fires from **`onExit`** — death stripping the buff — and whose flag also spares every *other* buff through death, exactly like Noblesse Blessing). Sub-slice 17 closed S4 with `NightStatModify` (Shadow Sense 294 — the stat belongs to the **clock**, not the buff, so `game_loop::night_stats` rewrites the landed buff's modifiers on every day/night flip rather than putting a clock in the stat hot path). Effect names 205 → 145, reachable 1303, headline 72 → 12. **S4 CLOSED** — the residue is 2 names / 10 learnable skills, of which `StatUp` (9) is out of scope and `SafeFallHeight` waits on fall damage existing at all; the rest are one-offs plus the `Pvp*DamageBonus` trio (needs `calculatePvpPveBonus` from scratch). Then S5–S8: `basicProperty`/`BasicPropertyResist`, the 23 missing `EffectFlag`s, the learnable-effect sweep, targeting breadth, the item/NPC tail (**every destination Scroll of Escape is inert today**), and the tag/formula residue. |
