...
- **Geodata editor** (`admin/world_cmds.rs` + `geo`): `//geomap`/`//geocell`
  ...
  `//geogrid [off]` draws/erases the NSWE arrow overlay (2026-08-01 — the
  one-shot `GeoUtils.debugGrid`/`hideDebugGrid`, sharing the Debug panel's
  renderer and its `DebugGrid_<n>` drawing names, exactly as Java's
  `AdminDebug.setGeodataDebugging` re-issues `admin_geogrid` on its timer);
  only the `//geoedit`/`//ge` community-board cell panel stays a stub.
...
- **Deferred**: only the `//geosave` binary-region serializer + the
  `//geoedit` cell panel remain simplified. Still blocked: clan-skill grants (no
