...
|---|---|---|
| 2026-08-07 | `TODO(G30)` — **the last marker. The inventory is empty: 180 → 0.** | **Closed — implemented to the reference's own depth, which is shallower than the names suggest.** `_bbsloc` renders the nine regions off the castles (name fstring, owner clan + alliance, buy tax) — its per-region detail is `// TODO: Implement.` **in Java itself**, as are the Mail/Memo/Friends writes and the home page's `getRegionCount` (hence `%region_count% = 0` is parity, and `%clan_count%` now reports the real clan count). `_maillist`/`_bbsmemo`/`_friendlist` serve their html shells. The one board with real machinery is the **clan board**: the paginated clan list (Java's own over-wide page bound kept), the clan home page (SM 1050 + fall-back to the list under level 2), and the clan **notice** — edit/enable/disable plus the `Write Notice Set` `RequestBBSwrite` command, stored in `clan_notices` (loaded at boot, saved through the new `SaveClanNotice`), and popped up to members at login (`clanNotice.htm`, newlines folded to `<br>` as Java does). All unreachable in production under `CustomCommunityBoard = True`; ported per the config-disabled rule. |
| 2026-08-07 | `TODO(G17)` | **Closed — two of its five items had been superseded in place, two are implemented, one argued.** Per-subclass hennas and shortcuts were already end-to-end (banked per index on the switch, loaded and stored per `class_index` — the marker's "both still load with class_index = 0" was stale). Implemented: the village-master cancel/replace flow (cases 3/6/7) — `modify_subclass` wipes the slot in memory (banked skills/hennas/shortcuts) and in the DB (the new `WipeSubclassSlot` command), lands the replacement in the freed index, and switches onto it; Java's sharp edge is kept and tested (the old subclass is gone even when the replacement fails — the player reverts to base). The Fate's Whisper + Mimir's Elixir completion gate (noble exempt) is ported behind `AltSubClassWithoutQuests` — True on this dist, so inert. Argued out with evidence: certification skills (no `subClassSkillTree` exists anywhere in this dist's data) and Java's `_subclassLock` (a two-thread interleave guard the single-threaded game loop cannot need). |
| 2026-08-07 | `TODO(G33)` | **Closed — implemented.** The `AltGameCreation` staged craft (Java `RecipeItemMaker` under `_activeMakers`): validation parks an `ActiveCraft` session on the crafter, each pass pays its per-pass share of the recipe's HP/MP (waiting on the gauge when short, consuming nothing), "equips" up to skill-level × GIM material units with SM 368, and animates the create skill into the next pass; the last grab glides the gauge into the settle — fee, materials, the success roll, and on success Java's ALT XP/SP award (reference-price ÷ recipe level, `altStatChange` overrides, the rare rate, 4× decay per skill level over the recipe, the three config knobs). `Recipes.xml`'s `altStatChange` (XP/SP/GIM) now parses; the five `AltGameCreation*` Character.ini keys landed. The marker's own site is real now: `is_crafting` (the `ActiveCraft` presence) gates the offline-craft mode exactly where Java's `isCrafting()` does. Config-disabled on this dist, ported per the house rule; tested with the config flipped on — staged parking, per-pass stat shares, grab messages, and the settle. |
| 2026-08-07 | `TODO(G15.5)` | **Closed — implemented.** The augment options' skill half: `Options.apply` now grants the option's **active** skills and registers its **activation** skills, not just the stat pumps. Both live in new transient components (`OptionSkills`, `OptionTriggers`) rather than the `SkillBook`, because Java grants them with `store = false` and this port persists the whole book — filed there they would outlive the item and re-arm on every login. The cast path's known-skill lookup consults both (`known_skill_level`), which is what makes an augment active *castable* rather than a bar button that answers `ActionFailed`. The procs fire at Java's two sites with Java's two predicates: `onHitTarget` splits `ATTACK` (non-crit) from `CRITICAL` (crit), and `SkillCaster` splits `MAGIC` (magic cast) from `ATTACK` (physical cast) with the whole block skipped for a static skill. On this dist that is 1793 `<active_skill>` rows and 688 triggers. **Found and fixed alongside:** `apply_item_options` only ever ran off an equip packet, so an augmented item worn *through a relog* contributed nothing at all — the stat half included — until it was manually re-equipped; enter-world now replays the equipped items' options the way Java's `restoreCharData` re-runs its equip listeners. 7 tests, all five mechanisms sabotage-verified. |
| 2026-08-06 | `TODO(G24/G26)` | **Closed — implemented.** The full chamberlain console: reports (`receive_report`, the fortress page rendered as Java shows it with no fortresses loaded), door open/close and door/wall/trap upgrades (prices from the new Feature.ini keys; door HP scaled on the live entity and re-derived at boot, trap levels persisted — both through `global_variables`), banish-foreigners (the oust shape filtered by owner clan), the siege-clan list, products/buy, the lord's cloak + crown, and the five rentable **castle functions**: the recovery/other pages with Java's `<fstring>` slots, the confirm + `set_func` purchase flow paying from the buyer, the renewal task charging the clan warehouse each period (a warehouse that can't pay loses the function), the `tel1/tel2` teleport lists routed back through the quest bypass, and the 28-buff support menu gated on the NPC's own MP. Function *effects* wired at their consumers: HP/MP regen inside the own-clan castle zone (`baseValue *= lvl/100` — **integer division, so the MP levels 40/55 multiply by zero; Java's shipped bug, ported as behaviour**) and the revive exp-restore on the to-castle respawn. Functions are runtime-only exactly like the reference: Java's `initFunctions()` is commented out of the `Castle` constructor, so bought functions never survive a restart there either. |
