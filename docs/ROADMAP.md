...
### G34 — Skills, effects & abnormal-state parity (epic) 🚧 **S0–S3 landed, S4 nearly closed (16 sub-slices; headline 275 → 13)**
...
the heal ceiling, the bespoke `Bluff`/`Unsummon`/`DeathLink`, and the whole of
Unlock 27 — `OpenDoor` + `OpenChest` + the `DOOR_TREASURE` target type, which
was the last learnable entry on the target axis — the sustain family
(`ChameleonRest`, `ManaHealOverTime`, `RebalanceHP`), and the fifteen-name
PvP/PvE balance family behind `calculatePvpPveBonus`, a term in every damage
...
`ImmobilePetBuff`, `CallParty`), and the death pair (`ReduceDropPenalty`,
`ResurrectionSpecial`); effect names **216 → 146**, reachable **1304**,
headline **275 → 13**; the residue is **3 names** — `StatUp` and
`SafeFallHeight` deliberately out of scope, leaving only `NightStatModify`);
targeting breadth
